/**
 * Created by bromanelli on 12/10/2016.
 */

import { Range } from 'immutable';

/*
export const saveMaptile = maptile =>  ({ firebase }) => {
  firebase.firebase();
  firebase.child(`users-presence/4f8vev89qz798vz87`)
    .push({
      authenticatedAt: 'hoho',
      user: 'haha',
    });
};
*/
export const MAPTILE_COMPLETED = 'MAPTILE_COMPLETED';
export const FIREBASE_SAVE_MAPTILE = 'FIREBASE_SAVE_MAPTILE';
export const DELETE_USER = 'DELETE_USER';
export const ADD_DEFAULT_DUNGEON = 'ADD_DEFAULT_DUNGEON';


// CREATE + SAVE VERSION
export const deleteUser = (viewer) => ({ firebase }) => {
    firebase.update({
        [`activeDungeons/${viewer.id}`]:null,
        [`activeMap/${viewer.id}`]:null,
        [`users/${viewer.id}/active`]:null,
        [`users/${viewer.id}/characters`]:null,

    });
    return {
        type: DELETE_USER,
        payload: null,
    };
};

export const addDefaultDungeon = () => ({ firebase, getUid}) => {

    // firebase.update({
    //     [`dungeons/Dungeon1`]: { description: "Dungeon 1", name: "Dungeon 1", from_editor : false,id:"Dungeon1",lock:false,worldmap:"a024ef95-98b8-4a7a-abfe-55a838c0e0c8", next: "Dungeon2" },
    //     [`dungeons/Dungeon2`]: { description: "Dungeon 2", name: "Dungeon 2", from_editor : false,id:"Dungeon2",lock:true,worldmap:"24f49f73-b3f0-4bed-b938-031b70f54a24", next: "Dungeon3" },
    //     [`dungeons/Dungeon3`]: { description: "Dungeon 3", name: "Dungeon 3", from_editor : false,id:"Dungeon3",lock:true,worldmap:"24f49f73-b3f0-4bed-b938-031b70f54a24", next: false },
    // });

    var b = getUid();

    firebase.update({
        [`monsters/${b}`]:
            {
                damage: 1000,
                experience : 950,
                health : 40000,
                maxhealth : 40000,
                image : "/assets/images/monsters/boss-cedric.png",
                id:b,
                movement:4,
                name : "MrDescoutures",
                range:3,
                type:"pnj" ,
            },
    });

    /*var a = getUid();
    var b = getUid();
    var c = getUid();
    var d = getUid();
    var e = getUid();
    var f = getUid();
    var g = getUid();
    var h = getUid();
    var i = getUid();
    var j = getUid();
    var k = getUid();
    var l = getUid();
    var m = getUid();
    var n = getUid();
    var o = getUid();
    var p = getUid();
    var q = getUid();
    var r = getUid();
    var s = getUid();
    var t = getUid();
    var u = getUid();
    var v = getUid();
    var w = getUid();
    var x = getUid();
    var y = getUid();
    var z = getUid();

    firebase.update({
        [`maptiles/${a}`]: {
            completed: false,
            id: a,
            image: "/assets/images/maptiles/coin-droite-bas-new.png",
            title: "Grey ground",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${b}`]: {
            completed: false,
            id: b,
            image: "/assets/images/maptiles/coin-droite-haut-new.png",
            title: "Grey ground",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${c}`]: {
            completed: false,
            id: c,
            image: "/assets/images/maptiles/coin-gauche-bas-new.png",
            title: "Grey ground",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${d}`]: {
            completed: false,
            id: d,
            image: "/assets/images/maptiles/coin-gauche-haut-new.png",
            title: "Grey ground",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${e}`]: {
            completed: false,
            id: e,
            image: "/assets/images/maptiles/floor1.png",
            title: "Floor",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${f}`]: {
            completed: false,
            id: f,
            image: "/assets/images/maptiles/floor2.png",
            title: "Floor",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${g}`]: {
            completed: false,
            id: g,
            image: "/assets/images/maptiles/floor3.png",
            title: "Floor",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${h}`]: {
            completed: false,
            id: h,
            image: "/assets/images/maptiles/floor4.png",
            title: "Floor",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${i}`]: {
            completed: false,
            id: i,
            image: "/assets/images/maptiles/sol-new.png",
            title: "Floor",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${j}`]: {
            completed: false,
            id: j,
            image: "/assets/images/maptiles/sol2-new.png",
            title: "Floor",
            type: "walkable",
        }});
    firebase.update({
        [`maptiles/${k}`]: {
            completed: false,
            id: k,
            image: "/assets/images/maptiles/mur-bas-new.png",
            title: "Grey wall",
            type: "wall",
        }});
    firebase.update({
        [`maptiles/${l}`]: {
            completed: false,
            id: l,
            image: "/assets/images/maptiles/mur-droite-new.png",
            title: "Grey wall",
            type: "wall",
        }});
    firebase.update({
        [`maptiles/${m}`]: {
            completed: false,
            id: m,
            image: "/assets/images/maptiles/mur-gauche-new.png",
            title: "Grey wall",
            type: "wall",
        }});
    firebase.update({
        [`maptiles/${n}`]: {
            completed: false,
            id: n,
            image: "/assets/images/maptiles/mur-haut-new.png",
            title: "Grey wall",
            type: "wall",
        }});*/


    return {
        type: ADD_DEFAULT_DUNGEON,
        payload: null,
    };
};

export const maptileCompleted = () => ({ getUid, now, firebase, firebaseDatabase }) => {
var a = getUid();
var b = getUid();
var c = getUid();
var d = getUid();
var e = getUid();
var f = getUid();
var g = getUid();
var h = getUid();
var i = getUid();

    firebase.update({
        [`classes/Mage/learned_spells/${a}`]: {
            damage_time_duration: 0,
            get: true,
            id: a,
            css: "objet0",
            name:"Solar eruption",
            image: "eruption.jpg",
            damage_instant: 500,
            damage_time: 0,
            damage_instant_buff: 0,
            damage_instant_debuff: 0,
            damage_type: "lightning",
            damage_debuff_flat: 0,
            damage_buff_flat: 0,
            damage_debuff_percent: 0,
            damage_buff_percent: 0,
            damage_reduction_flat: 0,
            damage_reduction_percent: 0,
            damage_time_buff_flat: 0,
            damage_time_buff_percent: 0,
            damage_time_debuff_flat: 0,
            damage_time_debuff_percent: 0,
            damage_return: 0,
            damage_return_percent: 0,
            life_buff: 0,
            life_debuff: 0,
            heal_instant: 0,
            heal_time: 0,
            heal_on_energy_percent: 0,
            heal_percent_instant: 0,
            heal_percent_time: 0,
            movement_instant: 0,
            movement_buff: 0,
            movement_debuff: 0,
            energy_heal: 0,
            energy_time: 0,
            energy_percent_heal: 0,
            energy_percent_time: 0,
            range_minimum: 0,
            range_linear: 2,
            range_diagonal: 1,
            aoe_linear: 0,
            aoe_diagonal: 0,
            range_cone: false,
            range_on_target: false,
            aoe_front: false,
            aoe_back: false,
            aoe_right: false,
            aoe_left: false,
            can_break: false,
            self: false,
            cast_time: 0,
            duration: 0,
            energy_cost: 500,
            life_cost: 0,
            action_cost: 5,
            uses: -1,
            rest: 0,
            description: "Launch a solar eruption to kill your enemy",
        }});
    firebase.update({
        [`classes/Mage/learned_spells/${b}`]: {
            damage_time_duration: 0,
            get: true,
            id: b,
            css: "objet0",
            name:"Speed lightning",
            image: "lighning.jpg",
            damage_instant: 200,
            damage_time: 0,
            damage_instant_buff: 0,
            damage_instant_debuff: 0,
            damage_type: "lightning",
            damage_debuff_flat: 0,
            damage_buff_flat: 0,
            damage_debuff_percent: 0,
            damage_buff_percent: 0,
            damage_reduction_flat: 0,
            damage_reduction_percent: 0,
            damage_time_buff_flat: 0,
            damage_time_buff_percent: 0,
            damage_time_debuff_flat: 0,
            damage_time_debuff_percent: 0,
            damage_return: 0,
            damage_return_percent: 0,
            life_buff: 0,
            life_debuff: 0,
            heal_instant: 0,
            heal_time: 0,
            heal_on_energy_percent: 0,
            heal_percent_instant: 0,
            heal_percent_time: 0,
            movement_instant: 0,
            movement_buff: 0,
            movement_debuff: 0,
            energy_heal: 0,
            energy_time: 0,
            energy_percent_heal: 0,
            energy_percent_time: 0,
            range_minimum: 0,
            range_linear: 2,
            range_diagonal: 1,
            aoe_linear: 0,
            aoe_diagonal: 0,
            range_cone: false,
            range_on_target: false,
            aoe_front: false,
            aoe_back: false,
            aoe_right: false,
            aoe_left: false,
            can_break: false,
            self: false,
            cast_time: 0,
            duration: 0,
            energy_cost: 300,
            life_cost: 0,
            action_cost: 2,
            uses: -1,
            rest: 0,
            description: "Launch a speed lightning to surprise your ennemy",
        }});
    firebase.update({
        [`classes/Mage/learned_spells/${c}`]: {
            damage_time_duration: 0,
            get: true,
            id: c,
            css: "objet0",
            name:"Spell gale",
            image: "gate.jpg",
            damage_instant: 500,
            damage_time: 0,
            damage_instant_buff: 0,
            damage_instant_debuff: 0,
            damage_type: "lightning",
            damage_debuff_flat: 0,
            damage_buff_flat: 0,
            damage_debuff_percent: 0,
            damage_buff_percent: 0,
            damage_reduction_flat: 0,
            damage_reduction_percent: 0,
            damage_time_buff_flat: 0,
            damage_time_buff_percent: 0,
            damage_time_debuff_flat: 0,
            damage_time_debuff_percent: 0,
            damage_return: 0,
            damage_return_percent: 0,
            life_buff: 0,
            life_debuff: 0,
            heal_instant: 0,
            heal_time: 0,
            heal_on_energy_percent: 0,
            heal_percent_instant: 0,
            heal_percent_time: 0,
            movement_instant: 0,
            movement_buff: 0,
            movement_debuff: 0,
            energy_heal: 0,
            energy_time: 0,
            energy_percent_heal: 0,
            energy_percent_time: 0,
            range_minimum: 0,
            range_linear: 2,
            range_diagonal: 0,
            aoe_linear: 2,
            aoe_diagonal: 1,
            range_cone: false,
            range_on_target: false,
            aoe_front: true,
            aoe_back: false,
            aoe_right: false,
            aoe_left: false,
            can_break: false,
            self: false,
            cast_time: 0,
            duration: 0,
            energy_cost: 800,
            life_cost: 0,
            action_cost: 10,
            uses: -1,
            rest: 2,
            description: "A spell to break enemy armor",
        }});
    firebase.update({
        [`classes/Ranger/learned_spells/${d}`]: {
            damage_time_duration: 0,
            get: true,
            id: d,
            css: "objet0",
            name:"Favor wind",
            image: "favorwind.jpg",
            damage_instant: 0,
            damage_time: 0,
            damage_instant_buff: 0,
            damage_instant_debuff: 0,
            damage_type: "Heal",
            damage_debuff_flat: 0,
            damage_buff_flat: 0,
            damage_debuff_percent: 0,
            damage_buff_percent: 0,
            damage_reduction_flat: 0,
            damage_reduction_percent: 0,
            damage_time_buff_flat: 0,
            damage_time_buff_percent: 0,
            damage_time_debuff_flat: 0,
            damage_time_debuff_percent: 0,
            damage_return: 0,
            damage_return_percent: 0,
            life_buff: 0,
            life_debuff: 0,
            heal_instant: 500,
            heal_time: 0,
            heal_on_energy_percent: 0,
            heal_percent_instant: 0,
            heal_percent_time: 0,
            movement_instant: 0,
            movement_buff: 0,
            movement_debuff: 0,
            energy_heal: 0,
            energy_time: 0,
            energy_percent_heal: 0,
            energy_percent_time: 0,
            range_minimum: 0,
            range_linear: 1,
            range_diagonal: 0,
            aoe_linear: 0,
            aoe_diagonal: 0,
            range_cone: false,
            range_on_target: false,
            aoe_front: false,
            aoe_back: false,
            aoe_right: false,
            aoe_left: false,
            can_break: false,
            self: true,
            cast_time: 0,
            duration: 0,
            energy_cost: 300,
            life_cost: 0,
            action_cost: 10,
            uses: -1,
            rest: 0,
            description: "Nature Ritual to speed your energy life",
        }});
    firebase.update({
        [`classes/Ranger/learned_spells/${e}`]: {
            damage_time_duration: 0,
            get: true,
            id: e,
            css: "objet0",
            name:"Track shoot",
            image: "trackshoot.jpg",
            damage_instant: 0,
            damage_time: 200,
            damage_instant_buff: 0,
            damage_instant_debuff: 0,
            damage_type: "Bleed",
            damage_debuff_flat: 0,
            damage_buff_flat: 0,
            damage_debuff_percent: 0,
            damage_buff_percent: 0,
            damage_reduction_flat: 0,
            damage_reduction_percent: 0,
            damage_time_buff_flat: 0,
            damage_time_buff_percent: 0,
            damage_time_debuff_flat: 0,
            damage_time_debuff_percent: 0,
            damage_return: 0,
            damage_return_percent: 0,
            life_buff: 0,
            life_debuff: 0,
            heal_instant: 0,
            heal_time: 0,
            heal_on_energy_percent: 0,
            heal_percent_instant: 0,
            heal_percent_time: 0,
            movement_instant: 0,
            movement_buff: 0,
            movement_debuff: 0,
            energy_heal: 0,
            energy_time: 0,
            energy_percent_heal: 0,
            energy_percent_time: 0,
            range_minimum: 0,
            range_linear: 4,
            range_diagonal: 3,
            aoe_linear: 0,
            aoe_diagonal: 0,
            range_cone: false,
            range_on_target: false,
            aoe_front: false,
            aoe_back: false,
            aoe_right: false,
            aoe_left: false,
            can_break: false,
            self: false,
            cast_time: 0,
            duration: 3,
            energy_cost: 200,
            life_cost: 0,
            action_cost: 10,
            uses: -1,
            rest: 4,
            description: "Bow Attack. Inflicts Bleeding condition if your target is hit",
        }});
    firebase.update({
        [`classes/Ranger/learned_spells/${f}`]: {
            damage_time_duration: 0,
            get: true,
            id: f,
            css: "objet0",
            name:"Focused Shoot",
            image: "focusedshoot.jpg",
            damage_instant: 0,
            damage_time: 0,
            damage_instant_buff: 1000,
            damage_instant_debuff: 0,
            damage_type: "",
            damage_debuff_flat: 0,
            damage_buff_flat: 0,
            damage_debuff_percent: 0,
            damage_buff_percent: 0,
            damage_reduction_flat: 0,
            damage_reduction_percent: 0,
            damage_time_buff_flat: 0,
            damage_time_buff_percent: 0,
            damage_time_debuff_flat: 0,
            damage_time_debuff_percent: 0,
            damage_return: 0,
            damage_return_percent: 0,
            life_buff: 0,
            life_debuff: 0,
            heal_instant: 0,
            heal_time: 0,
            heal_on_energy_percent: 0,
            heal_percent_instant: 0,
            heal_percent_time: 0,
            movement_instant: 0,
            movement_buff: 0,
            movement_debuff: 0,
            energy_heal: 0,
            energy_time: 0,
            energy_percent_heal: 0,
            energy_percent_time: 0,
            range_minimum: 0,
            range_linear: 2,
            range_diagonal: 2,
            aoe_linear: 0,
            aoe_diagonal: 0,
            range_cone: false,
            range_on_target: false,
            aoe_front: false,
            aoe_back: false,
            aoe_right: false,
            aoe_left: false,
            can_break: false,
            self: false,
            cast_time: 0,
            duration: 0,
            energy_cost: 800,
            life_cost: 0,
            action_cost: 10,
            uses: -1,
            rest: 10,
            description: "Bow Attack. To deal hugh damage attack",
        }});
    firebase.update({
        [`classes/Warrior/learned_spells/${g}`]: {
            damage_time_duration: 0,
            get: true,
            id: g,
            css: "objet0",
            name:"Fire sword",
            image: "firesword.jpg",
            damage_instant: 0,
            damage_time: 0,
            damage_instant_buff: 0,
            damage_instant_debuff: 0,
            damage_type: "Buff",
            damage_debuff_flat: 300,
            damage_buff_flat: 0,
            damage_debuff_percent: 0,
            damage_buff_percent: 0,
            damage_reduction_flat: 0,
            damage_reduction_percent: 0,
            damage_time_buff_flat: 0,
            damage_time_buff_percent: 0,
            damage_time_debuff_flat: 0,
            damage_time_debuff_percent: 0,
            damage_return: 0,
            damage_return_percent: 0,
            life_buff: 0,
            life_debuff: 0,
            heal_instant: 0,
            heal_time: 0,
            heal_on_energy_percent: 0,
            heal_percent_instant: 0,
            heal_percent_time: 0,
            movement_instant: 0,
            movement_buff: 0,
            movement_debuff: 0,
            energy_heal: 0,
            energy_time: 0,
            energy_percent_heal: 0,
            energy_percent_time: 0,
            range_minimum: 0,
            range_linear: 1,
            range_diagonal: 0,
            aoe_linear: 0,
            aoe_diagonal: 0,
            range_cone: false,
            range_on_target: false,
            aoe_front: false,
            aoe_back: false,
            aoe_right: false,
            aoe_left: false,
            can_break: false,
            self: true,
            cast_time: 0,
            duration: 6,
            energy_cost: 400,
            life_cost: 0,
            action_cost: 0,
            uses: -1,
            rest: 5,
            description: "Upgrade your weapon with fire to deal more damage",
        }});
    firebase.update({
        [`classes/Warrior/learned_spells/${h}`]: {
            damage_time_duration: 0,
            get: true,
            id: h,
            css: "objet0",
            name:"Shield up",
            image: "shieldup.jpg",
            damage_instant: 0,
            damage_time: 0,
            damage_instant_buff: 0,
            damage_instant_debuff: 0,
            damage_type: "lightning",
            damage_debuff_flat: 0,
            damage_buff_flat: 0,
            damage_debuff_percent: 0,
            damage_buff_percent: 0,
            damage_reduction_flat: 100,
            damage_reduction_percent: 0,
            damage_time_buff_flat: 0,
            damage_time_buff_percent: 0,
            damage_time_debuff_flat: 0,
            damage_time_debuff_percent: 0,
            damage_return: 100,
            damage_return_percent: 10,
            life_buff: 0,
            life_debuff: 0,
            heal_instant: 0,
            heal_time: 0,
            heal_on_energy_percent: 0,
            heal_percent_instant: 0,
            heal_percent_time: 0,
            movement_instant: 0,
            movement_buff: 0,
            movement_debuff: 0,
            energy_heal: 0,
            energy_time: 0,
            energy_percent_heal: 0,
            energy_percent_time: 0,
            range_minimum: 0,
            range_linear: 1,
            range_diagonal: 0,
            aoe_linear: 0,
            aoe_diagonal: 0,
            range_cone: false,
            range_on_target: false,
            aoe_front: false,
            aoe_back: false,
            aoe_right: false,
            aoe_left: false,
            can_break: false,
            self: true,
            cast_time: 0,
            duration: 5,
            energy_cost: 300,
            life_cost: 0,
            action_cost: 0,
            uses: -1,
            rest: 5,
            description: "Shout. You will return damage against your enemy.",
        }});
    firebase.update({
        [`classes/Warrior/learned_spells/${i}`]: {
            damage_time_duration: 0,
            get: true,
            id: i,
            css: "objet0",
            name:"Strike weapon",
            image: "strikehit.jpg",
            damage_instant: 0,
            damage_time: 0,
            damage_instant_buff: 200,
            damage_instant_debuff: 0,
            damage_type: "",
            damage_debuff_flat: 0,
            damage_buff_flat: 0,
            damage_debuff_percent: 0,
            damage_buff_percent: 0,
            damage_reduction_flat: 0,
            damage_reduction_percent: 0,
            damage_time_buff_flat: 0,
            damage_time_buff_percent: 0,
            damage_time_debuff_flat: 0,
            damage_time_debuff_percent: 0,
            damage_return: 0,
            damage_return_percent: 0,
            life_buff: 0,
            life_debuff: 0,
            heal_instant: 0,
            heal_time: 0,
            heal_on_energy_percent: 0,
            heal_percent_instant: 0,
            heal_percent_time: 0,
            movement_instant: 0,
            movement_buff: 0,
            movement_debuff: 0,
            energy_heal: 0,
            energy_time: 0,
            energy_percent_heal: 0,
            energy_percent_time: 0,
            range_minimum: 0,
            range_linear: 3,
            range_diagonal: 0,
            aoe_linear: 3,
            aoe_diagonal: 0,
            range_cone: false,
            range_on_target: false,
            aoe_front: true,
            aoe_back: false,
            aoe_right: false,
            aoe_left: false,
            can_break: false,
            self: false,
            cast_time: 0,
            duration: 0,
            energy_cost: 50,
            life_cost: 350,
            action_cost: 10,
            uses: -1,
            rest: 0,
            description: "Strike a hugh attack on ground to deal damage on enemy",
        }});

    // firebase.update({
    //     [`monsters/${id}`]:
    //         {
    //             damage: 400,
    //             experience : 950,
    //             health : 1800,
    //             maxhealth : 1800,
    //             image : "Sasquatch.png",
    //             id:id,
    //             movement:4,
    //             name : "Sasquatch",
    //             range:2,
    //             type:"pnj" ,
    //             is_boss:false
    //         },
    // });
    /*
    firebase.update({
        [`classes/Mage`]:
            {
                "action" : 10,
                "action_lvl" : 0.05,
                "armor" : 0,
                "basicCost" : 10,
                "damage" : 10,
                "damage_lvl" : 1,
                "damage_reduction_flat" : 0,
                "damage_reduction_flat_lvl" : 0,
                "damage_reduction_percent" : 0,
                "damage_reduction_percent_lvl" : 0,
                "damage_return" : 0,
                "damage_return_lvl" : 0,
                "damage_return_percent" : 0,
                "damage_time" : 0,
                "damage_time_duration" : 0,
                "description" : " Mages demolish their foes with fire incantations. Although they wield powerful offensive spells, mages are fragile and lightly armored, making them particularly vulnerable to close-range attacks. Wise mages make careful use of their spells to keep their foes at a distance or hold them in place.",
                "energy" : 12000,
                "energy_lvl" : 1250,
                "equipped_spells" : [
                    {
                    "action_cost" : 5,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet17",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 0,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Buff",
                    "description" : "Grant some life when using spells",
                    "duration" : 20,
                    "energy_cost" : 600,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 25,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "47ce1fb7-0206-407e-942b-daf9c5a898d7",
                    "image" : "Aura_of_Restoration.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Life conservation",
                    "number" : 0,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 0,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 10,
                    "self" : true,
                    "uses" : -1
                }, {
                    "action_cost" : 5,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet18",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 100,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Fire",
                    "description" : "Hit with a fire first !",
                    "duration" : 0,
                    "energy_cost" : 100,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "ffcda306-712e-4201-ae51-ae396f1d303b",
                    "image" : "Flare.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Fire fist",
                    "number" : 1,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 2,
                    "range_minimum" : 1,
                    "range_on_target" : false,
                    "rest" : 0,
                    "self" : false,
                    "uses" : -1
                }, {
                    "action_cost" : 5,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet19",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 0,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 250,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 2,
                    "damage_type" : "Mark",
                    "description" : "Mark your ennemy to deal more fire damage",
                    "duration" : 10,
                    "energy_cost" : 600,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "fd9d5180-ca9d-4ba2-bde0-7235f6824571",
                    "image" : "Mark_of_Rodgort.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Demon's mark",
                    "number" : 2,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 2,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 5,
                    "self" : false,
                    "uses" : -1
                }, {
                    "action_cost" : 10,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet20",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 1000,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Fire",
                    "description" : "Burn opponent to the ground",
                    "duration" : 0,
                    "energy_cost" : 800,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "1c4570d6-26aa-42ec-b48f-6a453d264811",
                    "image" : "Rodgort's_Invocation.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Demon's flame",
                    "number" : 3,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 2,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 0,
                    "self" : false,
                    "uses" : -1
                }, {
                    "action_cost" : 5,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet21",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 600,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Fire",
                    "description" : "Launch a fireball",
                    "duration" : 0,
                    "energy_cost" : 700,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "964571f5-6fd3-441c-811f-05f6ee133a4d",
                    "image" : "Fireball.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Fireball",
                    "number" : 4,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 3,
                    "range_minimum" : 3,
                    "range_on_target" : false,
                    "rest" : 0,
                    "self" : false,
                    "uses" : -1
                }, {
                    "action_cost" : 10,
                    "aoe_back" : false,
                    "aoe_diagonal" : 1,
                    "aoe_front" : true,
                    "aoe_left" : false,
                    "aoe_linear" : 1,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet22",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 300,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Fire",
                    "description" : "Create flame pikes from the ground",
                    "duration" : 0,
                    "energy_cost" : 250,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "b2cbbfa0-fd0e-40fd-afd5-e89f0181dac8",
                    "image" : "Flame_Burst.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Flame pikes",
                    "number" : 5,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 1,
                    "range_minimum" : 0,
                    "range_on_target" : true,
                    "rest" : 0,
                    "self" : false,
                    "uses" : -1
                }, {
                    "action_cost" : 10,
                    "aoe_back" : true,
                    "aoe_diagonal" : 1,
                    "aoe_front" : true,
                    "aoe_left" : true,
                    "aoe_linear" : 1,
                    "aoe_right" : true,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet23",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 1500,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Fire",
                    "description" : "Create a pillar of burning flames",
                    "duration" : 0,
                    "energy_cost" : 3000,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "66a545db-6bbc-4836-a8b6-a6d210543973",
                    "image" : "Immolate.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Flame pillar",
                    "number" : 6,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 3,
                    "range_minimum" : 0,
                    "range_on_target" : true,
                    "rest" : 3,
                    "self" : false,
                    "uses" : -1
                }, {
                    "action_cost" : 0,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet24",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 500,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Fire",
                    "description" : "Infinite flame rings crushing ennemies",
                    "duration" : 0,
                    "energy_cost" : 60,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "5a677dde-fb9c-4cee-bf0b-c172003844db",
                    "image" : "Searing_Heat.jpg",
                    "life_buff" : 0,
                    "life_cost" : 240,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Dancing flame rings",
                    "number" : 7,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 2,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 0,
                    "self" : false,
                    "uses" : -1
                } ],
                "experience" : 0,
                "health" : 3500,
                "health_lvl" : 150,
                "image" : "/assets/images/classes/Warrior/down.png",
                "learned_spells" : {
                    "47ce1fb7-0206-407e-942b-daf9c5a898d7": {
                        "action_cost": 5,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet17",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 0,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Buff",
                        "description": "Grant some life when using spells",
                        "duration": 20,
                        "energy_cost": 600,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 25,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "47ce1fb7-0206-407e-942b-daf9c5a898d7",
                        "image": "Aura_of_Restoration.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Life conservation",
                        "number": 0,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 0,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 10,
                        "self": true,
                        "uses": -1
                    },
                    "ffcda306-712e-4201-ae51-ae396f1d303b": {
                        "action_cost": 5,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet18",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 100,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Fire",
                        "description": "Hit with a fire first !",
                        "duration": 0,
                        "energy_cost": 100,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "ffcda306-712e-4201-ae51-ae396f1d303b",
                        "image": "Flare.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Fire fist",
                        "number": 1,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 2,
                        "range_minimum": 1,
                        "range_on_target": false,
                        "rest": 0,
                        "self": false,
                        "uses": -1
                    },
                    "fd9d5180-ca9d-4ba2-bde0-7235f6824571": {
                        "action_cost": 5,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet19",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 0,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 250,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 2,
                        "damage_type": "Mark",
                        "description": "Mark your ennemy to deal more fire damage",
                        "duration": 10,
                        "energy_cost": 600,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "fd9d5180-ca9d-4ba2-bde0-7235f6824571",
                        "image": "Mark_of_Rodgort.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Demon's mark",
                        "number": 2,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 2,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 5,
                        "self": false,
                        "uses": -1
                    },
                    "1c4570d6-26aa-42ec-b48f-6a453d264811": {
                        "action_cost": 10,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet20",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 1000,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Fire",
                        "description": "Burn opponent to the ground",
                        "duration": 0,
                        "energy_cost": 800,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "1c4570d6-26aa-42ec-b48f-6a453d264811",
                        "image": "Rodgort's_Invocation.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Demon's flame",
                        "number": 3,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 2,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 0,
                        "self": false,
                        "uses": -1
                    },
                    "964571f5-6fd3-441c-811f-05f6ee133a4d": {
                        "action_cost": 5,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet21",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 600,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Fire",
                        "description": "Launch a fireball",
                        "duration": 0,
                        "energy_cost": 700,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "964571f5-6fd3-441c-811f-05f6ee133a4d",
                        "image": "Fireball.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Fireball",
                        "number": 4,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 3,
                        "range_minimum": 3,
                        "range_on_target": false,
                        "rest": 0,
                        "self": false,
                        "uses": -1
                    },
                    "b2cbbfa0-fd0e-40fd-afd5-e89f0181dac8": {
                        "action_cost": 10,
                        "aoe_back": false,
                        "aoe_diagonal": 1,
                        "aoe_front": true,
                        "aoe_left": false,
                        "aoe_linear": 1,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet22",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 300,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Fire",
                        "description": "Create flame pikes from the ground",
                        "duration": 0,
                        "energy_cost": 250,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "b2cbbfa0-fd0e-40fd-afd5-e89f0181dac8",
                        "image": "Flame_Burst.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Flame pikes",
                        "number": 5,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 1,
                        "range_minimum": 0,
                        "range_on_target": true,
                        "rest": 0,
                        "self": false,
                        "uses": -1
                    },
                    "66a545db-6bbc-4836-a8b6-a6d210543973": {
                        "action_cost": 10,
                        "aoe_back": true,
                        "aoe_diagonal": 1,
                        "aoe_front": true,
                        "aoe_left": true,
                        "aoe_linear": 1,
                        "aoe_right": true,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet23",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 1500,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Fire",
                        "description": "Create a pillar of burning flames",
                        "duration": 0,
                        "energy_cost": 3000,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "66a545db-6bbc-4836-a8b6-a6d210543973",
                        "image": "Immolate.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Flame pillar",
                        "number": 6,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 3,
                        "range_minimum": 0,
                        "range_on_target": true,
                        "rest": 3,
                        "self": false,
                        "uses": -1
                    },
                    "5a677dde-fb9c-4cee-bf0b-c172003844db": {
                        "action_cost": 0,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet24",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 500,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Fire",
                        "description": "Infinite flame rings crushing ennemies",
                        "duration": 0,
                        "energy_cost": 60,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "5a677dde-fb9c-4cee-bf0b-c172003844db",
                        "image": "Searing_Heat.jpg",
                        "life_buff": 0,
                        "life_cost": 240,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Dancing flame rings",
                        "number": 7,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 2,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 0,
                        "self": false,
                        "uses": -1
                    },
                },
                "level" : 1,
                "maxexperience" : 5000,
                "movement" : 2,
                "movement_lvl" : 0.05,
                "name" : "Mage",
                "range" : 2,
                "range_lvl" : 0,
                "spell_damage" : 0,
                "sprites_name" : "mage",
                "type" : "pj"
            },
        [`classes/Ranger`]:
            {
                "action" : 10,
                "action_lvl" : 0.05,
                "armor" : 0,
                "basicCost" : 10,
                "damage" : 80,
                "damage_lvl" : 10,
                "damage_reduction_flat" : 0,
                "damage_reduction_flat_lvl" : 0,
                "damage_reduction_percent" : 0,
                "damage_reduction_percent_lvl" : 0,
                "damage_return" : 0,
                "damage_return_lvl" : 0,
                "damage_return_percent" : 0,
                "damage_time" : 0,
                "damage_time_duration" : 0,
                "description" : "From an early age, the call of the wild draws some adventurers from the comfort of their homes into the unforgiving primal world outside. Those who endure become rangers. Masters of their environment, they are able to slip like ghosts through the trees and lay traps in the paths of their enemies.",
                "energy" : 4500,
                "energy_lvl" : 500,
                "equipped_spells" :
                    [
                        {
                    "action_cost" : 0,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet9",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 0,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 300,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Buff",
                    "description" : "Strengthen your skin with bear's coat",
                    "duration" : 5,
                    "energy_cost" : 1000,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "be082965-270b-4f10-a3f9-a0cb85195645",
                    "image" : "Bears_Defense.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Bear's defense",
                    "number" : 0,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 0,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 10,
                    "self" : true,
                    "uses" : 4
                }, {
                    "action_cost" : 0,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet10",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 0,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Buff",
                    "description" : "It close open wounds",
                    "duration" : 10,
                    "energy_cost" : 500,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 200,
                    "id" : "be5be3c8-bb6c-4e6b-9ef7-73f0c01ed3e0",
                    "image" : "Troll_Unguent.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Troll unguent",
                    "number" : 1,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 0,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 10,
                    "self" : true,
                    "uses" : -1
                }, {
                    "action_cost" : 0,
                    "aoe_back" : true,
                    "aoe_diagonal" : 1,
                    "aoe_front" : true,
                    "aoe_left" : true,
                    "aoe_linear" : 1,
                    "aoe_right" : true,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet11",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 500,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "",
                    "description" : "Burn those who get trapped",
                    "duration" : 0,
                    "energy_cost" : 700,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "dfe30c2a-12ad-4879-9dbb-d4cf42578e8a",
                    "image" : "Flame_Trap.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Fire trap",
                    "number" : 2,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 0,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 2,
                    "self" : true,
                    "uses" : -1
                }, {
                    "action_cost" : 0,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet12",
                    "damage_buff_flat" : 220,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 0,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Buff",
                    "description" : "Hit ennemies with inciendary arrows",
                    "duration" : 4,
                    "energy_cost" : 500,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "51a4efa7-e3c7-4567-8128-bf27d7265259",
                    "image" : "Incendiary_Arrows.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Flame Arrows",
                    "number" : 3,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 0,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 4,
                    "self" : true,
                    "uses" : -1
                }, {
                    "action_cost" : 0,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet13",
                    "damage_buff_flat" : 120,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 0,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 40,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 10,
                    "damage_type" : "Buff",
                    "description" : "Hit ennemies with poisoned arrows",
                    "duration" : 4,
                    "energy_cost" : 500,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "bc41bd9f-4699-4080-aa1b-7477393c8437",
                    "image" : "Apply_Poison.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Poisoned Arrows",
                    "number" : 4,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 0,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 4,
                    "self" : true,
                    "uses" : -1
                }, {
                    "action_cost" : 10,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet14",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 150,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 0,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "",
                    "description" : "You shoot two arrows simultaneously at target foe",
                    "duration" : 1,
                    "energy_cost" : 600,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "082c32e7-c0a9-4701-bc67-f6459452461a",
                    "image" : "Barrage.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Dual shot",
                    "number" : 5,
                    "range_cone" : false,
                    "range_diagonal" : 4,
                    "range_linear" : 4,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 3,
                    "self" : false,
                    "uses" : -1
                }, {
                    "action_cost" : 10,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet15",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 250,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 0,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "",
                    "description" : "Powerful shot to get through all kind of armor",
                    "duration" : 0,
                    "energy_cost" : 1600,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "495ad8e6-e364-49b1-9443-917712a013f1",
                    "image" : "Marauder's_Shot.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 0,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Marauder's shot",
                    "number" : 6,
                    "range_cone" : false,
                    "range_diagonal" : 4,
                    "range_linear" : 4,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 10,
                    "self" : false,
                    "uses" : -1
                }, {
                    "action_cost" : 0,
                    "aoe_back" : false,
                    "aoe_diagonal" : 0,
                    "aoe_front" : false,
                    "aoe_left" : false,
                    "aoe_linear" : 0,
                    "aoe_right" : false,
                    "can_break" : false,
                    "cast_time" : 0,
                    "css" : "objet15",
                    "damage_buff_flat" : 0,
                    "damage_buff_percent" : 0,
                    "damage_debuff_flat" : 0,
                    "damage_debuff_percent" : 0,
                    "damage_instant" : 0,
                    "damage_instant_buff" : 0,
                    "damage_instant_debuff" : 0,
                    "damage_reduction_flat" : 0,
                    "damage_reduction_percent" : 0,
                    "damage_return" : 0,
                    "damage_return_percent" : 0,
                    "damage_time" : 0,
                    "damage_time_buff_flat" : 0,
                    "damage_time_buff_percent" : 0,
                    "damage_time_debuff_flat" : 0,
                    "damage_time_debuff_percent" : 0,
                    "damage_time_duration" : 0,
                    "damage_type" : "Buff",
                    "description" : "Speed up with nature's force",
                    "duration" : 6,
                    "energy_cost" : 500,
                    "energy_heal" : 0,
                    "energy_percent_heal" : 0,
                    "energy_percent_time" : 0,
                    "energy_time" : 0,
                    "get" : true,
                    "heal_instant" : 0,
                    "heal_on_energy_percent" : 0,
                    "heal_percent_instant" : 0,
                    "heal_percent_time" : 0,
                    "heal_time" : 0,
                    "id" : "86d9ae9b-06a4-489b-8686-f309af33e276",
                    "image" : "Natural_Stride.jpg",
                    "life_buff" : 0,
                    "life_cost" : 0,
                    "life_debuff" : 0,
                    "movement_buff" : 1,
                    "movement_debuff" : 0,
                    "movement_instant" : 0,
                    "name" : "Natural Stride",
                    "number" : 7,
                    "range_cone" : false,
                    "range_diagonal" : 0,
                    "range_linear" : 0,
                    "range_minimum" : 0,
                    "range_on_target" : false,
                    "rest" : 8,
                    "self" : true,
                    "uses" : -1
                } ],
                "experience" : 0,
                "health" : 6000,
                "health_lvl" : 500,
                "image" : "/assets/images/classes/Warrior/down.png",
                "learned_spells" : {
                    "be082965-270b-4f10-a3f9-a0cb85195645": {
                        "action_cost": 0,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet9",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 0,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 300,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Buff",
                        "description": "Strengthen your skin with bear's coat",
                        "duration": 5,
                        "energy_cost": 1000,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "be082965-270b-4f10-a3f9-a0cb85195645",
                        "image": "Bears_Defense.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Bear's defense",
                        "number": 0,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 0,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 10,
                        "self": true,
                        "uses": 4
                    },
                    "be5be3c8-bb6c-4e6b-9ef7-73f0c01ed3e0": {
                        "action_cost": 0,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet10",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 0,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Buff",
                        "description": "It close open wounds",
                        "duration": 10,
                        "energy_cost": 500,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 200,
                        "id": "be5be3c8-bb6c-4e6b-9ef7-73f0c01ed3e0",
                        "image": "Troll_Unguent.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Troll unguent",
                        "number": 1,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 0,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 10,
                        "self": true,
                        "uses": -1
                    },
                    "dfe30c2a-12ad-4879-9dbb-d4cf42578e8a": {
                        "action_cost": 0,
                        "aoe_back": true,
                        "aoe_diagonal": 1,
                        "aoe_front": true,
                        "aoe_left": true,
                        "aoe_linear": 1,
                        "aoe_right": true,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet11",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 500,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "",
                        "description": "Burn those who get trapped",
                        "duration": 0,
                        "energy_cost": 700,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "dfe30c2a-12ad-4879-9dbb-d4cf42578e8a",
                        "image": "Flame_Trap.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Fire trap",
                        "number": 2,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 0,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 2,
                        "self": true,
                        "uses": -1
                    },
                    "51a4efa7-e3c7-4567-8128-bf27d7265259": {
                        "action_cost": 0,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet12",
                        "damage_buff_flat": 220,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 0,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Buff",
                        "description": "Hit ennemies with inciendary arrows",
                        "duration": 4,
                        "energy_cost": 500,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "51a4efa7-e3c7-4567-8128-bf27d7265259",
                        "image": "Incendiary_Arrows.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Flame Arrows",
                        "number": 3,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 0,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 4,
                        "self": true,
                        "uses": -1
                    },
                    "bc41bd9f-4699-4080-aa1b-7477393c8437": {
                        "action_cost": 0,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet13",
                        "damage_buff_flat": 120,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 0,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 40,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 10,
                        "damage_type": "Buff",
                        "description": "Hit ennemies with poisoned arrows",
                        "duration": 4,
                        "energy_cost": 500,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "bc41bd9f-4699-4080-aa1b-7477393c8437",
                        "image": "Apply_Poison.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Poisoned Arrows",
                        "number": 4,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 0,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 4,
                        "self": true,
                        "uses": -1
                    },
                    "082c32e7-c0a9-4701-bc67-f6459452461a": {
                        "action_cost": 10,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet14",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 150,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 0,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "",
                        "description": "You shoot two arrows simultaneously at target foe",
                        "duration": 1,
                        "energy_cost": 600,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "082c32e7-c0a9-4701-bc67-f6459452461a",
                        "image": "Barrage.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Dual shot",
                        "number": 5,
                        "range_cone": false,
                        "range_diagonal": 4,
                        "range_linear": 4,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 3,
                        "self": false,
                        "uses": -1
                    },
                    "495ad8e6-e364-49b1-9443-917712a013f1": {
                        "action_cost": 10,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet15",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 250,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 0,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "",
                        "description": "Powerful shot to get through all kind of armor",
                        "duration": 0,
                        "energy_cost": 1600,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "495ad8e6-e364-49b1-9443-917712a013f1",
                        "image": "Marauder's_Shot.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Marauder's shot",
                        "number": 6,
                        "range_cone": false,
                        "range_diagonal": 4,
                        "range_linear": 4,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 10,
                        "self": false,
                        "uses": -1
                    },
                    "86d9ae9b-06a4-489b-8686-f309af33e276": {
                        "action_cost": 0,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 0,
                        "aoe_right": false,
                        "can_break": false,
                        "cast_time": 0,
                        "css": "objet15",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 0,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "Buff",
                        "description": "Speed up with nature's force",
                        "duration": 6,
                        "energy_cost": 500,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "86d9ae9b-06a4-489b-8686-f309af33e276",
                        "image": "Natural_Stride.jpg",
                        "life_buff": 0,
                        "life_cost": 0,
                        "life_debuff": 0,
                        "movement_buff": 1,
                        "movement_debuff": 0,
                        "movement_instant": 0,
                        "name": "Natural Stride",
                        "number": 7,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 0,
                        "range_minimum": 0,
                        "range_on_target": false,
                        "rest": 8,
                        "self": true,
                        "uses": -1
                    },
                },
                "level" : 1,
                "maxexperience" : 5000,
                "movement" : 3,
                "movement_lvl" : 0.05,
                "name" : "Ranger",
                "range" : 4,
                "range_lvl" : 0,
                "spell_damage" : 0,
                "sprites_name" : "ranger",
                "type" : "pj"
            },

        [`classes/Warrior`]: {
                "action": 10,
                "action_lvl": 0.05,
                "armor": 0,
                "basicCost": 10,
                "damage": 100,
                "damage_lvl": 10,
                "damage_reduction_flat": 50,
                "damage_reduction_flat_lvl": 5,
                "damage_reduction_percent": 0,
                "damage_reduction_percent_lvl": 0,
                "damage_return": 0,
                "damage_return_lvl": 0,
                "damage_return_percent": 0,
                "damage_time": 0,
                "damage_time_duration": 0,
                "description": "For as long as war has raged, heroes from every race have aimed to master the art of battle. Warriors combine strength, leadership, and a vast knowledge of arms and armor to wreak havoc in glorious combat.",
                "energy": 1000,
                "energy_lvl": 100,
                "equipped_spells": [
                    {
                    "action_cost": 10,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 4,
                    "aoe_right": false,
                    "can_break": true,
                    "cast_time": 0,
                    "css": "objet7",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 2500,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "",
                    "description": "Use inner strengh to charge like a bull ! Can break some walls.",
                    "duration": 0,
                    "energy_cost": 500,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "c9863368-778c-4ba9-8252-1b7e23317bac",
                    "image": "Bulls_Charge.jpg",
                    "life_buff": 0,
                    "life_cost": 1500,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 4,
                    "name": "Bulls charge",
                    "number": 0,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 1,
                    "range_minimum": 1,
                    "range_on_target": false,
                    "rest": 10,
                    "self": false,
                    "uses": 2
                }, {
                    "action_cost": 10,
                    "aoe_back": true,
                    "aoe_diagonal": 1,
                    "aoe_front": true,
                    "aoe_left": true,
                    "aoe_linear": 1,
                    "aoe_right": true,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet1",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 400,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "",
                    "description": "You turn yourself into a slashing tornado",
                    "duration": 0,
                    "energy_cost": 400,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "d26a79ab-a020-4f79-af5f-fdb0c56a6a16",
                    "image": "Cyclone_Axe.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Cyclone slash",
                    "number": 1,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 0,
                    "self": true,
                    "uses": -1
                }, {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet3",
                    "damage_buff_flat": 1400,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Buff",
                    "description": "Chop your ennemy with a brutal swing !",
                    "duration": 2,
                    "energy_cost": 600,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "bfff9742-dc2d-4188-acc1-04aa799de74e",
                    "image": "Disrupting_Chop.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Disrupting chop",
                    "number": 2,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 5,
                    "self": true,
                    "uses": -1
                }, {
                    "action_cost": 5,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet3",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 150,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Bleed",
                    "description": "Cut opponent's veins and make him bleed",
                    "duration": 10,
                    "energy_cost": 150,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "c705d362-a1b6-415b-a16e-8d34497af67c",
                    "image": "Gash.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Gash",
                    "number": 3,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 1,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 0,
                    "self": false,
                    "uses": -1
                }, {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet5",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Heal",
                    "description": "healing signet make you recover from slight injuries",
                    "duration": 0,
                    "energy_cost": 250,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 1000,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "ce3c54ce-33fd-4f5a-9180-7b639f04c030",
                    "image": "Healing_Signet.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Healing signet",
                    "number": 4,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 0,
                    "self": true,
                    "uses": -1
                }, {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet6",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Buff",
                    "description": "RUN",
                    "duration": 3,
                    "energy_cost": 100,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "a7d83741-9ecc-4714-bc83-8cb152520e0e",
                    "image": "Sprint.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 2,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Sprint",
                    "number": 5,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 6,
                    "self": true,
                    "uses": -1
                }, {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet2",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 250,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Buff",
                    "description": "Forgot about pain and relax",
                    "duration": 3,
                    "energy_cost": 250,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "7fff18a7-1cc3-4a8f-bb5a-e3d5703cf511",
                    "image": "defy_pain.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Defy pain",
                    "number": 6,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 5,
                    "self": true,
                    "uses": -1
                }, {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet8",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 50,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Melee_Counter",
                    "description": "Counter every melee attack and return damage based on ennemy's attack damage",
                    "duration": 2,
                    "energy_cost": 100,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "4c85ee79-002e-47b1-8744-0cfc45dab1b2",
                    "image": "Counterattack.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Counter Attack",
                    "number": 7,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 5,
                    "self": true,
                    "uses": -1
                }
                ],
                "experience": 0,
                "health": 15000,
                "health_lvl": 1500,
                "image": "/assets/images/classes/Warrior/down.png",
                "learned_spells": {
                    "c9863368-778c-4ba9-8252-1b7e23317bac": {
                        "action_cost": 10,
                        "aoe_back": false,
                        "aoe_diagonal": 0,
                        "aoe_front": false,
                        "aoe_left": false,
                        "aoe_linear": 4,
                        "aoe_right": false,
                        "can_break": true,
                        "cast_time": 0,
                        "css": "objet7",
                        "damage_buff_flat": 0,
                        "damage_buff_percent": 0,
                        "damage_debuff_flat": 0,
                        "damage_debuff_percent": 0,
                        "damage_instant": 2500,
                        "damage_instant_buff": 0,
                        "damage_instant_debuff": 0,
                        "damage_reduction_flat": 0,
                        "damage_reduction_percent": 0,
                        "damage_return": 0,
                        "damage_return_percent": 0,
                        "damage_time": 0,
                        "damage_time_buff_flat": 0,
                        "damage_time_buff_percent": 0,
                        "damage_time_debuff_flat": 0,
                        "damage_time_debuff_percent": 0,
                        "damage_time_duration": 0,
                        "damage_type": "",
                        "description": "Use inner strengh to charge like a bull ! Can break some walls.",
                        "duration": 0,
                        "energy_cost": 500,
                        "energy_heal": 0,
                        "energy_percent_heal": 0,
                        "energy_percent_time": 0,
                        "energy_time": 0,
                        "get": true,
                        "heal_instant": 0,
                        "heal_on_energy_percent": 0,
                        "heal_percent_instant": 0,
                        "heal_percent_time": 0,
                        "heal_time": 0,
                        "id": "c9863368-778c-4ba9-8252-1b7e23317bac",
                        "image": "Bulls_Charge.jpg",
                        "life_buff": 0,
                        "life_cost": 1500,
                        "life_debuff": 0,
                        "movement_buff": 0,
                        "movement_debuff": 0,
                        "movement_instant": 4,
                        "name": "Bulls charge",
                        "number": 0,
                        "range_cone": false,
                        "range_diagonal": 0,
                        "range_linear": 1,
                        "range_minimum": 1,
                        "range_on_target": false,
                        "rest": 10,
                        "self": false,
                        "uses": 2
                    },
                "d26a79ab-a020-4f79-af5f-fdb0c56a6a16": {
                    "action_cost": 10,
                    "aoe_back": true,
                    "aoe_diagonal": 1,
                    "aoe_front": true,
                    "aoe_left": true,
                    "aoe_linear": 1,
                    "aoe_right": true,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet1",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 400,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "",
                    "description": "You turn yourself into a slashing tornado",
                    "duration": 0,
                    "energy_cost": 400,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "d26a79ab-a020-4f79-af5f-fdb0c56a6a16",
                    "image": "Cyclone_Axe.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Cyclone slash",
                    "number": 1,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 0,
                    "self": true,
                    "uses": -1
                },
                "bfff9742-dc2d-4188-acc1-04aa799de74e": {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet3",
                    "damage_buff_flat": 1400,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Buff",
                    "description": "Chop your ennemy with a brutal swing !",
                    "duration": 2,
                    "energy_cost": 600,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "bfff9742-dc2d-4188-acc1-04aa799de74e",
                    "image": "Disrupting_Chop.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Disrupting chop",
                    "number": 2,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 5,
                    "self": true,
                    "uses": -1
                },
                "c705d362-a1b6-415b-a16e-8d34497af67c": {
                    "action_cost": 5,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet3",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 150,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Bleed",
                    "description": "Cut opponent's veins and make him bleed",
                    "duration": 10,
                    "energy_cost": 150,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "c705d362-a1b6-415b-a16e-8d34497af67c",
                    "image": "Gash.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Gash",
                    "number": 3,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 1,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 0,
                    "self": false,
                    "uses": -1
                },
                "ce3c54ce-33fd-4f5a-9180-7b639f04c030": {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet5",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Heal",
                    "description": "healing signet make you recover from slight injuries",
                    "duration": 0,
                    "energy_cost": 250,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 1000,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "ce3c54ce-33fd-4f5a-9180-7b639f04c030",
                    "image": "Healing_Signet.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Healing signet",
                    "number": 4,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 0,
                    "self": true,
                    "uses": -1
                },
                "a7d83741-9ecc-4714-bc83-8cb152520e0e": {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet6",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Buff",
                    "description": "RUN",
                    "duration": 3,
                    "energy_cost": 100,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "a7d83741-9ecc-4714-bc83-8cb152520e0e",
                    "image": "Sprint.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 2,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Sprint",
                    "number": 5,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 6,
                    "self": true,
                    "uses": -1
                },
                "7fff18a7-1cc3-4a8f-bb5a-e3d5703cf511": {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet2",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 250,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 0,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Buff",
                    "description": "Forgot about pain and relax",
                    "duration": 3,
                    "energy_cost": 250,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "7fff18a7-1cc3-4a8f-bb5a-e3d5703cf511",
                    "image": "defy_pain.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Defy pain",
                    "number": 6,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 5,
                    "self": true,
                    "uses": -1
                },
                "4c85ee79-002e-47b1-8744-0cfc45dab1b2": {
                    "action_cost": 0,
                    "aoe_back": false,
                    "aoe_diagonal": 0,
                    "aoe_front": false,
                    "aoe_left": false,
                    "aoe_linear": 0,
                    "aoe_right": false,
                    "can_break": false,
                    "cast_time": 0,
                    "css": "objet8",
                    "damage_buff_flat": 0,
                    "damage_buff_percent": 0,
                    "damage_debuff_flat": 0,
                    "damage_debuff_percent": 0,
                    "damage_instant": 0,
                    "damage_instant_buff": 0,
                    "damage_instant_debuff": 0,
                    "damage_reduction_flat": 0,
                    "damage_reduction_percent": 0,
                    "damage_return": 0,
                    "damage_return_percent": 50,
                    "damage_time": 0,
                    "damage_time_buff_flat": 0,
                    "damage_time_buff_percent": 0,
                    "damage_time_debuff_flat": 0,
                    "damage_time_debuff_percent": 0,
                    "damage_time_duration": 0,
                    "damage_type": "Melee_Counter",
                    "description": "Counter every melee attack and return damage based on ennemy's attack damage",
                    "duration": 2,
                    "energy_cost": 100,
                    "energy_heal": 0,
                    "energy_percent_heal": 0,
                    "energy_percent_time": 0,
                    "energy_time": 0,
                    "get": true,
                    "heal_instant": 0,
                    "heal_on_energy_percent": 0,
                    "heal_percent_instant": 0,
                    "heal_percent_time": 0,
                    "heal_time": 0,
                    "id": "4c85ee79-002e-47b1-8744-0cfc45dab1b2",
                    "image": "Counterattack.jpg",
                    "life_buff": 0,
                    "life_cost": 0,
                    "life_debuff": 0,
                    "movement_buff": 0,
                    "movement_debuff": 0,
                    "movement_instant": 0,
                    "name": "Counter Attack",
                    "number": 7,
                    "range_cone": false,
                    "range_diagonal": 0,
                    "range_linear": 0,
                    "range_minimum": 0,
                    "range_on_target": false,
                    "rest": 5,
                    "self": true,
                    "uses": -1
                }
                },
                    "level" : 1,
                    "maxexperience" : 5000,
                    "movement" : 2,
                    "movement_lvl" : 0.05,
                    "name" : "Warrior",
                    "range" : 1,
                    "range_lvl" : 0,
                    "spell_damage" : 0,
                    "sprites_name" : "warrior",
                    "type" : "pj"
                },

    });
    */


    // firebase.update({
    //     [`items`]:
    //     {
    //         "871edd06-c9a0-4d71-8f33-01a5ac8e250d": {
    //             get: true,
    //             id: "871edd06-c9a0-4d71-8f33-01a5ac8e250d",
    //             css: "objet0",
    //             name:"Life potion",
    //             // animation: "spell1.gif",
    //             image: "2.jpg",
    //             damage_instant: 0,
    //             damage_time: 0,
    //             damage_instant_buff: 0,
    //             damage_instant_debuff: 0,
    //             damage_type: "Heal",
    //             damage_debuff_flat: 0,
    //             damage_buff_flat: 0,
    //             damage_debuff_percent: 0,
    //             damage_buff_percent: 0,
    //             damage_reduction_flat: 0,
    //             damage_reduction_percent: 0,
    //             damage_time_buff_flat: 0,
    //             damage_time_buff_percent: 0,
    //             damage_time_debuff_flat: 0,
    //             damage_time_debuff_percent: 0,
    //             damage_return: 0,
    //             damage_return_percent: 0,
    //             life_buff: 0,
    //             life_debuff: 0,
    //             heal_instant: 0,
    //             heal_time: 0,
    //             heal_on_energy_percent: 0,
    //             heal_percent_instant: 100,
    //             heal_percent_time: 0,
    //             movement_instant: 0,
    //             movement_buff: 0,
    //             movement_debuff: 0,
    //             energy_heal: 0,
    //             energy_time: 0,
    //             energy_percent_heal: 0,
    //             energy_percent_time: 0,
    //             range_minimum: 0,
    //             range_linear: 0,
    //             range_diagonal: 0,
    //             aoe_linear: 0,
    //             aoe_diagonal: 0,
    //             range_cone: false,
    //             range_on_target: false,
    //             aoe_front: false,
    //             aoe_back: false,
    //             aoe_right: false,
    //             aoe_left: false,
    //             can_break: false,
    //             self: true,
    //             cast_time: 0,
    //             duration: 0,
    //             damage_time_duration: 0,
    //             energy_cost: 0,
    //             life_cost: 0,
    //             action_cost: 0,
    //             uses: 1,
    //             rest: 0,
    //             description: "Full heal",
    //         },
    //         "31f257c6-552d-4f75-b131-7baf88191bb5": {
    //             get: true,
    //             id: "31f257c6-552d-4f75-b131-7baf88191bb5",
    //             css: "objet1",
    //             name:"Energy potion",
    //             // animation: "spell1.gif",
    //             image: "1.jpg",
    //             damage_instant: 0,
    //             damage_time: 0,
    //             damage_instant_buff: 0,
    //             damage_instant_debuff: 0,
    //             damage_type: "Heal",
    //             damage_debuff_flat: 0,
    //             damage_buff_flat: 0,
    //             damage_debuff_percent: 0,
    //             damage_buff_percent: 0,
    //             damage_reduction_flat: 0,
    //             damage_reduction_percent: 0,
    //             damage_time_buff_flat: 0,
    //             damage_time_buff_percent: 0,
    //             damage_time_debuff_flat: 0,
    //             damage_time_debuff_percent: 0,
    //             damage_return: 0,
    //             damage_return_percent: 0,
    //             life_buff: 0,
    //             life_debuff: 0,
    //             heal_instant: 0,
    //             heal_time: 0,
    //             heal_on_energy_percent: 0,
    //             heal_percent_instant: 0,
    //             heal_percent_time: 0,
    //             movement_instant: 0,
    //             movement_buff: 0,
    //             movement_debuff: 0,
    //             energy_heal: 0,
    //             energy_time: 0,
    //             energy_percent_heal: 100,
    //             energy_percent_time: 0,
    //             range_minimum: 0,
    //             range_linear: 0,
    //             range_diagonal: 0,
    //             aoe_linear: 0,
    //             aoe_diagonal: 0,
    //             range_cone: false,
    //             range_on_target: false,
    //             aoe_front: false,
    //             aoe_back: false,
    //             aoe_right: false,
    //             aoe_left: false,
    //             can_break: false,
    //             self: true,
    //             cast_time: 0,
    //             duration: 0,
    //             damage_time_duration: 0,
    //             energy_cost: 0,
    //             life_cost: 0,
    //             action_cost: 0,
    //             uses: 1,
    //             rest: 0,
    //             description: "Restore energy",
    //         },
    //         "ba7f89d3-265d-4f1a-a802-852e9949dcaf": {
    //             get: true,
    //             id: "ba7f89d3-265d-4f1a-a802-852e9949dcaf",
    //             css: "objet2",
    //             name:"Energy potion",
    //             // animation: "spell1.gif",
    //             image: "7.jpg",
    //             damage_instant: 0,
    //             damage_time: 0,
    //             damage_instant_buff: 0,
    //             damage_instant_debuff: 0,
    //             damage_type: "Buff",
    //             damage_debuff_flat: 0,
    //             damage_buff_flat: 250,
    //             damage_debuff_percent: 0,
    //             damage_buff_percent: 0,
    //             damage_reduction_flat: 50,
    //             damage_reduction_percent: 0,
    //             damage_time_buff_flat: 0,
    //             damage_time_buff_percent: 0,
    //             damage_time_debuff_flat: 0,
    //             damage_time_debuff_percent: 0,
    //             damage_return: 0,
    //             damage_return_percent: 0,
    //             life_buff: 0,
    //             life_debuff: 0,
    //             heal_instant: 0,
    //             heal_time: 0,
    //             heal_on_energy_percent: 0,
    //             heal_percent_instant: 0,
    //             heal_percent_time: 0,
    //             movement_instant: 0,
    //             movement_buff: 0,
    //             movement_debuff: 0,
    //             energy_heal: 0,
    //             energy_time: 0,
    //             energy_percent_heal: 0,
    //             energy_percent_time: 0,
    //             range_minimum: 0,
    //             range_linear: 0,
    //             range_diagonal: 0,
    //             aoe_linear: 0,
    //             aoe_diagonal: 0,
    //             range_cone: false,
    //             range_on_target: false,
    //             aoe_front: false,
    //             aoe_back: false,
    //             aoe_right: false,
    //             aoe_left: false,
    //             can_break: false,
    //             self: true,
    //             cast_time: 0,
    //             duration: 10,
    //             damage_time_duration: 0,
    //             energy_cost: 0,
    //             life_cost: 0,
    //             action_cost: 0,
    //             uses: 1,
    //             rest: 0,
    //             description: "Enrage yourself to deal more damage",
    //         },
    //         "29ec6789-662c-48ad-a71d-0de75fd15991": {
    //             get: true,
    //             id: "29ec6789-662c-48ad-a71d-0de75fd15991",
    //             css: "objet3",
    //             name:"Defensive potion",
    //             // animation: "spell1.gif",
    //             image: "6.jpg",
    //             damage_instant: 0,
    //             damage_time: 0,
    //             damage_instant_buff: 0,
    //             damage_instant_debuff: 0,
    //             damage_type: "Buff",
    //             damage_debuff_flat: 0,
    //             damage_buff_flat: 0,
    //             damage_debuff_percent: 0,
    //             damage_buff_percent: 0,
    //             damage_reduction_flat: 300,
    //             damage_reduction_percent: 0,
    //             damage_time_buff_flat: 0,
    //             damage_time_buff_percent: 0,
    //             damage_time_debuff_flat: 0,
    //             damage_time_debuff_percent: 0,
    //             damage_return: 200,
    //             damage_return_percent: 10,
    //             life_buff: 0,
    //             life_debuff: 0,
    //             heal_instant: 0,
    //             heal_time: 0,
    //             heal_on_energy_percent: 0,
    //             heal_percent_instant: 0,
    //             heal_percent_time: 0,
    //             movement_instant: 0,
    //             movement_buff: 0,
    //             movement_debuff: 0,
    //             energy_heal: 0,
    //             energy_time: 0,
    //             energy_percent_heal: 0,
    //             energy_percent_time: 0,
    //             range_minimum: 0,
    //             range_linear: 0,
    //             range_diagonal: 0,
    //             aoe_linear: 0,
    //             aoe_diagonal: 0,
    //             range_cone: false,
    //             range_on_target: false,
    //             aoe_front: false,
    //             aoe_back: false,
    //             aoe_right: false,
    //             aoe_left: false,
    //             can_break: false,
    //             self: true,
    //             cast_time: 0,
    //             duration: 10,
    //             damage_time_duration: 0,
    //             energy_cost: 0,
    //             life_cost: 0,
    //             action_cost: 0,
    //             uses: 1,
    //             rest: 0,
    //             description: "Grant a powerful ability to resist at damages",
    //         },
    //         "36e38167-8e33-4713-bd04-02b159eaf8e7": {
    //             get: true,
    //             id: "36e38167-8e33-4713-bd04-02b159eaf8e7",
    //             css: "objet4",
    //             name:"Quickness potion",
    //             // animation: "spell1.gif",
    //             image: "10.jpg",
    //             damage_instant: 0,
    //             damage_time: 0,
    //             damage_instant_buff: 0,
    //             damage_instant_debuff: 0,
    //             damage_type: "Buff",
    //             damage_debuff_flat: 0,
    //             damage_buff_flat: 0,
    //             damage_debuff_percent: 0,
    //             damage_buff_percent: 0,
    //             damage_reduction_flat: 0,
    //             damage_reduction_percent: 0,
    //             damage_time_buff_flat: 100,
    //             damage_time_buff_percent: 0,
    //             damage_time_debuff_flat: 0,
    //             damage_time_debuff_percent: 0,
    //             damage_return: 0,
    //             damage_return_percent: 0,
    //             life_buff: 0,
    //             life_debuff: 0,
    //             heal_instant: 0,
    //             heal_time: 0,
    //             heal_on_energy_percent: 0,
    //             heal_percent_instant: 0,
    //             heal_percent_time: 0,
    //             movement_instant: 0,
    //             movement_buff: 2,
    //             movement_debuff: 0,
    //             energy_heal: 0,
    //             energy_time: 0,
    //             energy_percent_heal: 0,
    //             energy_percent_time: 0,
    //             range_minimum: 0,
    //             range_linear: 0,
    //             range_diagonal: 0,
    //             aoe_linear: 0,
    //             aoe_diagonal: 0,
    //             range_cone: false,
    //             range_on_target: false,
    //             aoe_front: false,
    //             aoe_back: false,
    //             aoe_right: false,
    //             aoe_left: false,
    //             can_break: false,
    //             self: true,
    //             cast_time: 0,
    //             duration: 5,
    //             damage_time_duration: 3,
    //             energy_cost: 0,
    //             life_cost: 0,
    //             action_cost: 0,
    //             uses: 1,
    //             rest: 0,
    //             description: "Faster than flash !",
    //         },
    //         "24258630-4ac0-4276-8c2d-8a081ee5a70f": {
    //             get: true,
    //             id: "24258630-4ac0-4276-8c2d-8a081ee5a70f",
    //             css: "objet4",
    //             name:"Mixed potion",
    //             // animation: "spell1.gif",
    //             image: "4.jpg",
    //             damage_instant: 0,
    //             damage_time: 0,
    //             damage_instant_buff: 0,
    //             damage_instant_debuff: 0,
    //             damage_type: "Heal",
    //             damage_debuff_flat: 0,
    //             damage_buff_flat: 0,
    //             damage_debuff_percent: 0,
    //             damage_buff_percent: 0,
    //             damage_reduction_flat: 0,
    //             damage_reduction_percent: 0,
    //             damage_time_buff_flat: 0,
    //             damage_time_buff_percent: 0,
    //             damage_time_debuff_flat: 0,
    //             damage_time_debuff_percent: 0,
    //             damage_return: 0,
    //             damage_return_percent: 0,
    //             life_buff: 0,
    //             life_debuff: 0,
    //             heal_instant: 0,
    //             heal_time: 0,
    //             heal_on_energy_percent: 0,
    //             heal_percent_instant: 50,
    //             heal_percent_time: 0,
    //             movement_instant: 0,
    //             movement_buff: 0,
    //             movement_debuff: 0,
    //             energy_heal: 0,
    //             energy_time: 0,
    //             energy_percent_heal: 50,
    //             energy_percent_time: 0,
    //             range_minimum: 0,
    //             range_linear: 0,
    //             range_diagonal: 0,
    //             aoe_linear: 0,
    //             aoe_diagonal: 0,
    //             range_cone: false,
    //             range_on_target: false,
    //             aoe_front: false,
    //             aoe_back: false,
    //             aoe_right: false,
    //             aoe_left: false,
    //             can_break: false,
    //             self: true,
    //             cast_time: 0,
    //             duration: 0,
    //             damage_time_duration: 0,
    //             energy_cost: 0,
    //             life_cost: 0,
    //             action_cost: 0,
    //             uses: 1,
    //             rest: 0,
    //             description: "Restore half life and energy",
    //         },
    //     }
    // });
  //[`dungeons/${id}`]: { id: id, map:`987654321`, name : `Practice`,description : `Training dungeon` },
  // [`typeDungeons/${id}`]: { id: id, name : `Practice`,description : `Training dungeon`, idMap1: `603414e3-d8fc-4b3f-ac56-376160eb7958`},
  //[`activeDungeons/${id}`]: { createdAt: firebaseDatabase.ServerValue.TIMESTAMP, dungeon: dungeon},

    //ADD Monsters
    /*
    firebase.update({
        [`monsters/${id}`]: {
                id:id,
                image:"/assets/images/monsters/remove_monster_tile.png",
                damage:0,
                health:0,
                maxhealth:0,
                name:"Remove Monster",
                type:"remove",
                move: 0,
                range: 0
            },});
    */


  //ADD FAKE MAP
    /*


    firebase.update({
      [`maps/603414e3-d8fc-4b3f-ac56-376160eb7958`]: {
          id: '603414e3-d8fc-4b3f-ac56-376160eb7958', name: `first`,
          monsters : {
              0: {
                  image:"/assets/images/monsters/Warrior.png",
                  damage:250,
                  health:400,
                  maxhealth:400,
                  name:"Warrior",
                  type:"pnj",
                  row: 0,
                  col: 1,
                  move: 1,
                  range: 1,
                  is_attacking: false,
                  is_moving: false,
                  number:0,
              },
              1: {
                  image:"/assets/images/monsters/Warrior.png",
                  damage:250,
                  health:400,
                  maxhealth:400,
                  name:"Warrior",
                  type:"pnj",
                  row: 3,
                  col: 1,
                  move: 1,
                  range: 1,
                  is_attacking: false,
                  is_moving: false,
                  number:1,
              }
          },
          maptiles: {
              0: {
                  0: {
                      completed: false,
                      id: id1,
                      image: "/assets/images/grass.png",
                      title: "forest",
                      type: "walkable",
                      character: {
                          image:"/assets/images/classes/gface.png",
                          damage:100,
                          health:15000,
                          name:"Warrior",
                          type:"pj",
                      }
                  },
                  1: {
                      completed: false,
                      id: id2,
                      image: "/assets/images/grass.png",
                      title: "forest",
                      type: "walkable",
                      character: {
                          image:"/assets/images/monsters/Warrior.png",
                          damage:250,
                          health:400,
                          maxhealth:400,
                          name:"Warrior",
                          type:"pnj",
                          move: 1,
                          range: 1,
                          number:0
                      }
                  },
                  2: {
                      completed: false,
                      id: id3,
                      image: "/assets/images/forest.png",
                      title: "forest"
                  },
                  3: {
                    completed: false,
                    id: id4,
                    image: "/assets/images/forest.png",
                    title: "forest"
                  },
                  4: {
                    completed: false,
                    id: id5,
                    image: "/assets/images/forest.png",
                    title: "forest"
                  },
                  5: {
                    completed: false,
                    id: id6,
                    image: "/assets/images/forest.png",
                    title: "forest"
                  },
                  6: {
                    completed: false,
                    id: id7,
                    image: "/assets/images/forest.png",
                    title: "forest"
                  },
                  7: {
                    completed: false,
                    id: id8,
                    image: "/assets/images/forest.png",
                    title: "forest"
                  },
                  8: {
                    completed: false,
                    id: id9,
                    image: "/assets/images/forest.png",
                    title: "forest"
                  },
                  9: {
                    completed: false,
                    id: id10,
                    image: "/assets/images/forest.png",
                    title: "forest"
                  }
              },
              1: {
                0: {
                  completed: false,
                  id: id11,
                  image: "/assets/images/grass.png",
                    type: "walkable",
                  title: "forest"
                },
                1: {
                  completed: false,
                  id: id12,
                  image: "/assets/images/grass.png",
                    type: "walkable",
                  title: "forest"
                },
                2: {
                  completed: false,
                  id: id13,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                3: {
                  completed: false,
                  id: id14,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                4: {
                  completed: false,
                  id: id15,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                5: {
                  completed: false,
                  id: id16,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                6: {
                  completed: false,
                  id: id17,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                7: {
                  completed: false,
                  id: id18,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                8: {
                  completed: false,
                  id: id19,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                9: {
                  completed: false,
                  id: id20,
                  image: "/assets/images/forest.png",
                  title: "forest"
                }
              },
              2: {
                0: {
                  completed: false,
                  id: id21,
                  image: "/assets/images/grass.png",
                    type: "walkable",
                  title: "forest",
                },
                1: {
                  completed: false,
                  id: id22,
                  image: "/assets/images/grass.png",
                    type: "walkable",
                  title: "forest",
                },
                2: {
                  completed: false,
                  id: id23,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                3: {
                  completed: false,
                  id: id24,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                4: {
                  completed: false,
                  id: id25,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                5: {
                  completed: false,
                  id: id26,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                6: {
                  completed: false,
                  id: id27,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                7: {
                  completed: false,
                  id: id28,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                8: {
                  completed: false,
                  id: id29,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                9: {
                  completed: false,
                  id: id30,
                  image: "/assets/images/forest.png",
                  title: "forest"
                }
              },
              3: {
                0: {
                  completed: false,
                  id: id31,
                  image: "/assets/images/grass.png",
                    type: "walkable",
                  title: "forest"
                },
                1: {
                  completed: false,
                  id: id32,
                  image: "/assets/images/grass.png",
                    type: "walkable",
                  title: "forest",
                  character: {
                      image:"/assets/images/monsters/Warrior.png",
                      damage:250,
                      health:400,
                      maxhealth:400,
                      name:"Warrior",
                      type:"pnj",
                      move: 1,
                      range: 1,
                      number:1
                  }
                },
                2: {
                  completed: false,
                  id: id33,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                3: {
                  completed: false,
                  id: id34,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                4: {
                  completed: false,
                  id: id35,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                5: {
                  completed: false,
                  id: id36,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                6: {
                  completed: false,
                  id: id37,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                7: {
                  completed: false,
                  id: id38,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                8: {
                  completed: false,
                  id: id39,
                  image: "/assets/images/forest.png",
                  title: "forest"
                },
                9: {
                  completed: false,
                  id: id40,
                  image: "/assets/images/forest.png",
                  title: "forest"
                }
              },
            4: {
              0: {
                completed: false,
                id: id41,
                image: "/assets/images/grass.png",
                  type: "walkable",
                title: "forest"
              },
              1: {
                completed: false,
                id: id42,
                image: "/assets/images/grass.png",
                  type: "walkable",
                title: "forest"
              },
              2: {
                completed: false,
                id: id43,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              3: {
                completed: false,
                id: id44,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              4: {
                completed: false,
                id: id45,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              5: {
                completed: false,
                id: id46,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              6: {
                completed: false,
                id: id47,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              7: {
                completed: false,
                id: id48,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              8: {
                completed: false,
                id: id49,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              9: {
                completed: false,
                id: id50,
                image: "/assets/images/forest.png",
                title: "forest"
              }
            },
            5: {
              0: {
                completed: false,
                id: id51,
                image: "/assets/images/grass.png",
                  type: "walkable",
                title: "forest"
              },
              1: {
                completed: false,
                id: id52,
                image: "/assets/images/grass.png",
                  type: "walkable",
                title: "forest"
              },
              2: {
                completed: false,
                id: id53,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              3: {
                completed: false,
                id: id54,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              4: {
                completed: false,
                id: id55,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              5: {
                completed: false,
                id: id56,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              6: {
                completed: false,
                id: id57,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              7: {
                completed: false,
                id: id58,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              8: {
                completed: false,
                id: id59,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              9: {
                completed: false,
                id: id60,
                image: "/assets/images/forest.png",
                title: "forest"
              }
            },
            6: {
              0: {
                completed: false,
                id: id61,
                image: "/assets/images/grass.png",
                  type: "walkable",
                title: "forest"
              },
              1: {
                completed: false,
                id: id62,
                image: "/assets/images/grass.png",
                  type: "walkable",
                title: "forest"
              },
              2: {
                completed: false,
                id: id63,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              3: {
                completed: false,
                id: id64,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              4: {
                completed: false,
                id: id65,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              5: {
                completed: false,
                id: id66,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              6: {
                completed: false,
                id: id67,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              7: {
                completed: false,
                id: id68,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              8: {
                completed: false,
                id: id69,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              9: {
                completed: false,
                id: id70,
                image: "/assets/images/forest.png",
                title: "forest"
              }
            },
            7:{
              0: {
                completed: false,
                id: id71,
                image: "/assets/images/grass.png",
                  type: "walkable",
                title: "forest"
              },
              1: {
                completed: false,
                id: id72,
                image: "/assets/images/grass.png",
                  type: "walkable",
                title: "forest"
              },
              2: {
                completed: false,
                id: id73,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              3: {
                completed: false,
                id: id74,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              4: {
                completed: false,
                id: id75,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              5: {
                completed: false,
                id: id76,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              6: {
                completed: false,
                id: id77,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              7: {
                completed: false,
                id: id78,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              8: {
                completed: false,
                id: id79,
                image: "/assets/images/forest.png",
                title: "forest"
              },
              9: {
                completed: false,
                id: id80,
                image: "/assets/images/forest.png",
                title: "forest"
              }
            }
          }
      }});
*/

  //ADD FAKE SKILLS
    /*
  firebase.update({
    [`skills`]: {
      0: {
        get: true,
        id: "81b3a0a2-8810-4691-9c95-fa8313bee937",
        css: "objet0",
        name:"Lightning strike",
        // animation: "spell1.gif",
        image: "spell1.gif",
        damage_instant: 500,
        damage_time: 0,
        damage_instant_buff: 0,
        damage_instant_debuff: 0,
        damage_type: "lightning",
        damage_debuff_flat: 0,
        damage_buff_flat: 0,
        damage_debuff_percent: 0,
        damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
        damage_return: 0,
        damage_return_percent: 0,
        life_buff: 0,
        life_debuff: 0,
        heal_instant: 0,
        heal_time: 0,
          heal_on_energy_percent: 0,
        heal_percent_instant: 0,
        heal_percent_time: 0,
        movement_instant: 0,
        movement_buff: 0,
        movement_debuff: 0,
        energy_heal: 0,
        energy_time: 0,
        energy_percent_heal: 0,
        energy_percent_time: 0,
        range_minimum: 0,
        range_linear: 2,
        range_diagonal: 0,
        aoe_linear: 0,
        aoe_diagonal: 0,
        range_cone: false,
        range_on_target: false,
        aoe_front: false,
        aoe_back: false,
        aoe_right: false,
        aoe_left: false,
        can_break: false,
        self: false,
        cast_time: 0,
        duration: 0,
        energy_cost: 500,
        life_cost: 0,
        action_cost: 5,
        uses: -1,
        rest: 0,
        description: "A lightning come from your hands and strike your ennemy",
      },
      1: {
          get: true,
          id: "d26a79ab-a020-4f79-af5f-fdb0c56a6a16",
          css: "objet1",
          name:"Cyclone slash",
          // animation: "spell1.gif",
          image: "Cyclone_Axe.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 400,
          damage_instant_debuff: 0,
          damage_type: "",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 1,
          aoe_diagonal: 1,
          range_cone: false,
          range_on_target: false,
          aoe_front: true,
          aoe_back: true,
          aoe_right: true,
          aoe_left: true,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 0,
          energy_cost: 400,
          life_cost: 0,
          action_cost: 10,
          uses: -1,
          rest: 0,
          description: "You turn yourself into a slashing tornado",
      },
      2: {
          get: true,
          id: "7fff18a7-1cc3-4a8f-bb5a-e3d5703cf511",
          css: "objet2",
          name:"Defy pain",
          // animation: "spell1.gif",
          image: "defy_pain.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 250,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 0,
          duration: 3,
          energy_cost: 250,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 5,
          description: "Forgot about pain and relax",
      },
      3: {
          get: true,
          id: "bfff9742-dc2d-4188-acc1-04aa799de74e",
          css: "objet3",
          name:"Disrupting chop",
          // animation: "spell1.gif",
          image: "Disrupting_Chop.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Buff",
          damage_debuff_flat: 0,
          damage_buff_flat: 1400,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 0,
          duration: 2,
          energy_cost: 600,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 5,
          description: "Chop your ennemy with a brutal swing !",
      },
      4: {
          get: true,
          id: "c705d362-a1b6-415b-a16e-8d34497af67c",
          css: "objet3",
          name:"Gash",
          // animation: "spell1.gif",
          image: "Gash.jpg",
          damage_instant: 0,
          damage_time: 150,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Bleed",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 1,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 10,
          energy_cost: 150,
          life_cost: 0,
          action_cost: 5,
          uses: -1,
          rest: 0,
          description: "Cut opponent's veins and make him bleed",
      },
        //Healing_Signet
      5: {
          get: true,
          id: "ce3c54ce-33fd-4f5a-9180-7b639f04c030",
          css: "objet5",
          name:"Healing signet",
          // animation: "spell1.gif",
          image: "Healing_Signet.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Heal",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 1000,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 0,
          duration: 0,
          energy_cost: 250,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 0,
          description: "healing signet make you recover from slight injuries",
      },
      6: {
          get: true,
          id: "a7d83741-9ecc-4714-bc83-8cb152520e0e",
          css: "objet6",
          name:"Sprint",
          // animation: "spell1.gif",
          image: "Sprint.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Buff",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 2,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 0,
          duration: 3,
          energy_cost: 100,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 6,
          description: "RUN",
      },
      7: {
          get: true,
          id: "c9863368-778c-4ba9-8252-1b7e23317bac",
          css: "objet7",
          name:"Bulls charge",
          // animation: "spell1.gif",
          image: "Bulls_Charge.jpg",
          damage_instant: 2500,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 4,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 1,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 4,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: true,
          self: false,
          cast_time: 0,
          duration: 0,
          energy_cost: 500,
          life_cost: 1500,
          action_cost: 10,
          uses: 2,
          rest: 10,
          description: "Use inner strengh to charge like a bull ! Can break some walls.",
      },
      8: {
          get: true,
          id: "4c85ee79-002e-47b1-8744-0cfc45dab1b2",
          css: "objet8",
          name:"Counter Attack",
          // animation: "spell1.gif",
          image: "Counterattack.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Melee_Counter",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 50,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 0,
          duration: 2,
          energy_cost: 100,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 5,
          description: "Counter every melee attack and return damage based on ennemy's attack damage",
      },
      9: {
          get: true,
          id: "be082965-270b-4f10-a3f9-a0cb85195645",
          css: "objet9",
          name:"Bear's defense",
          // animation: "spell1.gif",
          image: "Bears_Defense.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Buff",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 300,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 0,
          duration: 5,
          energy_cost: 1000,
          life_cost: 0,
          action_cost: 0,
          uses: 4,
          rest: 10,
          description: "Strengthen your skin with bear's coat",
      },
      10: {
          get: true,
          id: "be5be3c8-bb6c-4e6b-9ef7-73f0c01ed3e0",
          css: "objet10",
          name:"Troll unguent",
          // animation: "spell1.gif",
          image: "Troll_Unguent.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Buff",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 200,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 0,
          duration: 10,
          energy_cost: 500,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 10,
          description: "It close open wounds",
      },
      11: {
          get: true,
          id: "dfe30c2a-12ad-4879-9dbb-d4cf42578e8a",
          css: "objet11",
          name:"Fire trap",
          // animation: "spell1.gif",
          image: "Flame_Trap.jpg",
          damage_instant: 500,
          damage_time: 30,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Trap",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 1,
          aoe_diagonal: 1,
          range_cone: false,
          range_on_target: false,
          aoe_front: true,
          aoe_back: true,
          aoe_right: true,
          aoe_left: true,
          can_break: false,
          self: true,
          cast_time: 2,
          duration: 10,
          energy_cost: 700,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 2,
          description: "Burn those who get trapped",
      },
      12: {
          get: true,
          id: "51a4efa7-e3c7-4567-8128-bf27d7265259",
          css: "objet12",
          name:"Flame Arrows",
          // animation: "spell1.gif",
          image: "Incendiary_Arrows.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Buff",
          damage_debuff_flat: 0,
          damage_buff_flat: 220,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 1,
          duration: 4,
          damage_time_duration: 0,
          energy_cost: 500,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 4,
          description: "Hit ennemies with inciendary arrows",
      },
      13: {
          get: true,
          id: "bc41bd9f-4699-4080-aa1b-7477393c8437",
          css: "objet13",
          name:"Poisoned Arrows",
          // animation: "spell1.gif",
          image: "Apply_Poison.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Buff",
          damage_debuff_flat: 0,
          damage_buff_flat: 120,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 40,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 1,
          duration: 4,
          damage_time_duration: 10,
          energy_cost: 500,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 4,
          description: "Hit ennemies with poisoned arrows",
      },
      14: {
          get: true,
          id: "082c32e7-c0a9-4701-bc67-f6459452461a",
          css: "objet14",
          name:"Dual shot",
          // animation: "spell1.gif",
          image: "Barrage.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 150,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 4,
          range_diagonal: 3,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 1,
          damage_time_duration: 0,
          energy_cost: 600,
          life_cost: 0,
          action_cost: 10,
          uses: -1,
          rest: 3,
          description: "You shoot two arrows simultaneously at target foe",
      },
      15: {
          get: true,
          id: "495ad8e6-e364-49b1-9443-917712a013f1",
          css: "objet15",
          name:"Marauder's shot",
          // animation: "spell1.gif",
          image: "Marauder's_Shot.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 250,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 4,
          range_diagonal: 3,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 0,
          damage_time_duration: 0,
          energy_cost: 1600,
          life_cost: 0,
          action_cost: 10,
          uses: -1,
          rest: 10,
          description: "Powerful shot to get through all kind of armor",
      },
      16: {
          get: true,
          id: "86d9ae9b-06a4-489b-8686-f309af33e276",
          css: "objet16",
          name:"Natural Stride",
          // animation: "spell1.gif",
          image: "Natural_Stride.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Buff",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 1,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 0,
          duration: 6,
          damage_time_duration: 0,
          energy_cost: 500,
          life_cost: 0,
          action_cost: 0,
          uses: -1,
          rest: 8,
          description: "Speed up with nature's force",
      },
      17: {
          get: true,
          id: "47ce1fb7-0206-407e-942b-daf9c5a898d7",
          css: "objet17",
          name:"Life conservation",
          // animation: "spell1.gif",
          image: "Aura_of_Restoration.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Buff",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 25,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: true,
          cast_time: 0,
          duration: 20,
          damage_time_duration: 0,
          energy_cost: 600,
          life_cost: 0,
          action_cost: 5,
          uses: -1,
          rest: 10,
          description: "Grant some life when using spells",
      },
      18: {
          get: true,
          id: "ffcda306-712e-4201-ae51-ae396f1d303b",
          css: "objet18",
          name:"Fire fist",
          // animation: "spell1.gif",
          image: "Flare.jpg",
          damage_instant: 100,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 0,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 0,
          damage_time_duration: 0,
          energy_cost: 100,
          life_cost: 0,
          action_cost: 5,
          uses: -1,
          rest: 0,
          description: "Hit with a fire first !",
      },
      19: {
          get: true,
          id: "fd9d5180-ca9d-4ba2-bde0-7235f6824571",
          css: "objet19",
          name:"Demon's mark",
          // animation: "spell1.gif",
          image: "Mark_of_Rodgort.jpg",
          damage_instant: 0,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Mark",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 250,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 2,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 10,
          damage_time_duration: 2,
          energy_cost: 600,
          life_cost: 0,
          action_cost: 5,
          uses: -1,
          rest: 5,
          description: "Mark your ennemy to deal more fire damage",
      },
      20: {
          get: true,
          id: "1c4570d6-26aa-42ec-b48f-6a453d264811",
          css: "objet20",
          name:"Demon's flame",
          // animation: "spell1.gif",
          image: "Rodgort's_Invocation.jpg",
          damage_instant: 1000,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Fire",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 2,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 0,
          damage_time_duration: 0,
          energy_cost: 800,
          life_cost: 0,
          action_cost: 10,
          uses: -1,
          rest: 0,
          description: "Burn opponent to the ground",
      },
      21: {
          get: true,
          id: "964571f5-6fd3-441c-811f-05f6ee133a4d",
          css: "objet21",
          name:"Fireball",
          // animation: "spell1.gif",
          image: "Fireball.jpg",
          damage_instant: 600,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Fire",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 3,
          range_linear: 3,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 0,
          damage_time_duration: 0,
          energy_cost: 700,
          life_cost: 0,
          action_cost: 5,
          uses: -1,
          rest: 0,
          description: "Launch a fireball",
      },
      22: {
          get: true,
          id: "b2cbbfa0-fd0e-40fd-afd5-e89f0181dac8",
          css: "objet22",
          name:"Flame pikes",
          // animation: "spell1.gif",
          image: "Flame_Burst.jpg",
          damage_instant: 300,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Fire",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 1,
          range_diagonal: 0,
          aoe_linear: 2,
          aoe_diagonal: 1,
          range_cone: false,
          range_on_target: false,
          aoe_front: true,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 0,
          damage_time_duration: 0,
          energy_cost: 250,
          life_cost: 0,
          action_cost: 10,
          uses: -1,
          rest: 0,
          description: "Create flame pikes from the ground",
      },
      23: {
          get: true,
          id: "66a545db-6bbc-4836-a8b6-a6d210543973",
          css: "objet23",
          name:"Flame pillar",
          // animation: "spell1.gif",
          image: "Immolate.jpg",
          damage_instant: 1500,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Fire",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 3,
          range_diagonal: 0,
          aoe_linear: 1,
          aoe_diagonal: 1,
          range_cone: false,
          range_on_target: true,
          aoe_front: true,
          aoe_back: true,
          aoe_right: true,
          aoe_left: true,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 0,
          damage_time_duration: 0,
          energy_cost: 3000,
          life_cost: 0,
          action_cost: 10,
          uses: -1,
          rest: 3,
          description: "Create a pillar of burning flames",
      },
      24: {
          get: true,
          id: "5a677dde-fb9c-4cee-bf0b-c172003844db",
          css: "objet24",
          name:"Dancing flame rings",
          // animation: "spell1.gif",
          image: "Searing_Heat.jpg",
          damage_instant: 500,
          damage_time: 0,
          damage_instant_buff: 0,
          damage_instant_debuff: 0,
          damage_type: "Fire",
          damage_debuff_flat: 0,
          damage_buff_flat: 0,
          damage_debuff_percent: 0,
          damage_buff_percent: 0,
          damage_reduction_flat: 0,
          damage_reduction_percent: 0,
          damage_time_buff_flat: 0,
          damage_time_buff_percent: 0,
          damage_time_debuff_flat: 0,
          damage_time_debuff_percent: 0,
          damage_return: 0,
          damage_return_percent: 0,
          life_buff: 0,
          life_debuff: 0,
          heal_instant: 0,
          heal_time: 0,
          heal_on_energy_percent: 0,
          heal_percent_instant: 0,
          heal_percent_time: 0,
          movement_instant: 0,
          movement_buff: 0,
          movement_debuff: 0,
          energy_heal: 0,
          energy_time: 0,
          energy_percent_heal: 0,
          energy_percent_time: 0,
          range_minimum: 0,
          range_linear: 2,
          range_diagonal: 0,
          aoe_linear: 0,
          aoe_diagonal: 0,
          range_cone: false,
          range_on_target: false,
          aoe_front: false,
          aoe_back: false,
          aoe_right: false,
          aoe_left: false,
          can_break: false,
          self: false,
          cast_time: 0,
          duration: 0,
          damage_time_duration: 0,
          energy_cost: 60,
          life_cost: 240,
          action_cost: 0,
          uses: -1,
          rest: 0,
          description: "Infinite flame rings crushing ennemies",
      },
    }
  });*/

  /*//ADD FAKE WEAPONS
  var a = getUid();
  var b = getUid();
  var c = getUid();
  var d = getUid();
  var e = getUid();
  var f = getUid();
  var g = getUid();
  var h = getUid();
  var i = getUid();
  var j = getUid();
  var k = getUid();
  var l = getUid();
  var m = getUid();
  var n = getUid();
  var o = getUid();
  var p = getUid();
  var q = getUid();
  var r = getUid();
  var s = getUid();
  var t = getUid();
  var u = getUid();
  var v = getUid();
  var w = getUid();
  var x = getUid();
  var y = getUid();
  var z = getUid();
  firebase.update({
    [`equipments/${a}`]: {
            name: a,
            img: "/assets/images/weapons/armor_1.png",
            type: "armor",
            classe: "Mage",
            benefits: {
                health:  100,
                energy: 1000,
                spell_damage: 500
            },

        }});

    firebase.update({
        [`users/iZndq6gNXyOeYsS8xDHenTyMQJe2/characters/0/inventory/${a}`]: {
            name: a,
            img: "/assets/images/weapons/armor_1.png",
            type: "armor",
            classe: "Mage",
            benefits: {
                health:  100,
                energy: 1000,
                spell_damage: 500
            },

        }});

    firebase.update({
        [`equipments/${b}`]: {
            name: b,
            img: "/assets/images/weapons/weapon_1.png",
            type: "weapon",
            classe: "Mage",
            benefits: {
                health:  1000,
                energy: 1000,
                spell_damage: 500
            },

        }});

    firebase.update({
        [`users/iZndq6gNXyOeYsS8xDHenTyMQJe2/characters/0/inventory/${b}`]: {
            name: b,
            img: "/assets/images/weapons/weapon_1.png",
            type: "weapon",
            classe: "Mage",
            benefits: {
                health:  1000,
                energy: 1000,
                spell_damage: 500
            },

        }});

    firebase.update({
        [`equipments/${c}`]: {
            name: c,
            img: "/assets/images/weapons/armor_2.png",
            type: "armor",
            classe: "Mage",
            benefits: {
                energy: 3000,
                spell_damage: 1500
            },

        }});

    firebase.update({
        [`users/iZndq6gNXyOeYsS8xDHenTyMQJe2/characters/0/inventory/${c}`]: {
            name: c,
            img: "/assets/images/weapons/armor_2.png",
            type: "armor",
            classe: "Mage",
            benefits: {
                energy: 3000,
                spell_damage: 1500
            },

        }});

    firebase.update({
        [`equipments/${d}`]: {
            name: d,
            img: "/assets/images/weapons/weapon_2.png",
            type: "weapon",
            classe: "Mage",
            benefits: {
                energy: 1000,
                spell_damage: 2000
            },

        }});

    firebase.update({
        [`users/iZndq6gNXyOeYsS8xDHenTyMQJe2/characters/0/inventory/${d}`]: {
            name: d,
            img: "/assets/images/weapons/weapon_2.png",
            type: "weapon",
            classe: "Mage",
            benefits: {
                energy: 1000,
                spell_damage: 2000
            },

        }});

    firebase.update({
        [`equipments/${e}`]: {
            name: e,
            img: "/assets/images/weapons/helmet_1.png",
            type: "helmet",
            classe: "Mage",
            benefits: {
                energy: 1000,
                spell_damage: 1000
            },

        }});

    firebase.update({
        [`users/iZndq6gNXyOeYsS8xDHenTyMQJe2/characters/0/inventory/${e}`]: {
            name: e,
            img: "/assets/images/weapons/helmet_1.png",
            type: "helmet",
            classe: "Mage",
            benefits: {
                energy: 1000,
                spell_damage: 1000
            },

        }});

    firebase.update({
        [`equipments/${f}`]: {
            name: f,
            img: "/assets/images/weapons/boots_6.png",
            type: "boots",
            classe: "Mage",
            benefits: {
                energy: 500,
                spell_damage: 500
            },

        }});

    firebase.update({
        [`equipments/${g}`]: {
            name: g,
            img: "/assets/images/weapons/armor_3.png",
            type: "armor",
            classe: "Mage",
            benefits: {
                energy: 3000,
                spell_damage: 300
            },

        }});

    firebase.update({
        [`equipments/${h}`]: {
            name: h,
            img: "/assets/images/weapons/boots_3.png",
                type: "boots",
                classe: "Mage",
                benefits: {
                    energy: 3000,
                    spell_damage: 300
                },

        }});

    firebase.update({
        [`equipments/${i}`]: {
            name: i,
            img: "/assets/images/weapons/helmet_2.png",
            type: "helmet",
            classe: "Mage",
            benefits: {
                energy: 2000,
                spell_damage: 2000
            },

        }});

    firebase.update({
        [`equipments/${j}`]: {
            name: j,
            img: "/assets/images/weapons/weapon_3.png",
                type: "weapon",
                classe: "Mage",
                benefits: {
                    energy: 5000,
                    spell_damage: 5000
                },
        }});

    firebase.update({
        [`equipments/${k}`]: {
            name: k,
            img: "/assets/images/weapons/weapon_4.png",
                type: "weapon",
                classe: "Ranger",
                benefits: {
                    health: 1000,
                    energy: 1000,
                    damage: 1000
                },
        }});

    firebase.update({
        [`equipments/${l}`]: {
            name: l,
            img: "/assets/images/weapons/armor_6.png",
            type: "armor",
            classe: "Ranger",
            benefits: {
                health: 1000,
                energy: 1000,
                damage: 1000
            },
        }});

    firebase.update({
        [`equipments/${m}`]: {
            name: m,
            img: "/assets/images/weapons/boots_3.png",
            type: "boots",
            classe: "Ranger",
            benefits: {
                health: 1000,
                energy: 1000,
                damage: 1000
            },
        }});

    firebase.update({
        [`equipments/${n}`]: {
            name: n,
            img: "/assets/images/weapons/helmet_5.png",
            type: "helmet",
            classe: "Ranger",
            benefits: {
                health: 1000,
                energy: 1000,
                damage: 1000
            },
        }});

    firebase.update({
        [`equipments/${o}`]: {
            name: o,
            img: "/assets/images/weapons/weapon_5.png",
            type: "weapon",
            classe: "Ranger",
            benefits: {
                health: 1500,
                energy: 1500,
                damage: 1500
            },
        }});

    firebase.update({
        [`equipments/${p}`]: {
            name: p,
            img: "/assets/images/weapons/armor_7.png",
            type: "armor",
            classe: "Ranger",
            benefits: {
                health: 1500,
                energy: 1500,
                damage: 1500
            },
        }});

    firebase.update({
        [`equipments/${q}`]: {
            name: q,
            img: "/assets/images/weapons/boots_4.png",
            type: "boots",
            classe: "Ranger",
            benefits: {
                health: 1500,
                energy: 1500,
                damage: 1500
            },
        }});

    firebase.update({
        [`equipments/${r}`]: {
            name: r,
            img: "/assets/images/weapons/helmet_6.png",
            type: "helmet",
            classe: "Ranger",
            benefits: {
                health: 1500,
                energy: 1500,
                damage: 1500
            },
        }});


    firebase.update({
        [`equipments/${s}`]: {
            name: s,
            img: "/assets/images/weapons/weapon_7.png",
            type: "weapon",
            classe: "Warrior",
            benefits: {
                damage: 1000,
                health: 1500,
                damage_reduction_flat: 200,
                armor: 200
            },
        }});

    firebase.update({
        [`equipments/${t}`]: {
            name: t,
            img: "/assets/images/weapons/armor_4.png",
            type: "armor",
            classe: "Warrior",
            benefits: {
                damage: 1000,
                health: 2000,
                damage_reduction_flat: 200,
                armor: 200
            },
        }});

    firebase.update({
        [`equipments/${u}`]: {
            name: u,
            img: "/assets/images/weapons/boots_1.png",
            type: "boots",
            classe: "Warrior",
            benefits: {
                damage: 1000,
                health: 2000,
                damage_reduction_flat: 200,
                armor: 200
            },
        }});

    firebase.update({
        [`equipments/${v}`]: {
            name: v,
            img: "/assets/images/weapons/helmet_3.png",
            type: "helmet",
            classe: "Warrior",
            benefits: {
                damage: 1000,
                health: 2000,
                damage_reduction_flat: 200,
                armor: 200
            },
        }});

    firebase.update({
        [`equipments/${w}`]: {
            name: w,
            img: "/assets/images/weapons/weapon_8.png",
            type: "weapon",
            classe: "Warrior",
            benefits: {
                damage: 1000,
                health: 1500,
                energy: 1500,
                damage_reduction_flat: 200,
            },
        }});

    firebase.update({
        [`equipments/${w}`]: {
            name: w,
            img: "/assets/images/weapons/helmet_4.png",
            type: "helmet",
            classe: "Warrior",
            benefits: {
                damage: 1000,
                health: 1500,
                energy: 1500,
                damage_return: 200,
            },
        }});

    firebase.update({
        [`equipments/${x}`]: {
            name: x,
            img: "/assets/images/weapons/armor_5.png",
            type: "armor",
            classe: "Warrior",
            benefits: {
                damage: 1500,
                health: 1000,
                armor: 200,
                damage_return: 200,
            },
        }});

    firebase.update({
        [`equipments/${y}`]: {
            name: y,
            img: "/assets/images/weapons/boots_2.png",
            type: "boots",
            classe: "Warrior",
            benefits: {
                damage: 1500,
                health: 1000,
                armor: 200,
                damage_return: 200,
            },
        }});

    firebase.update({
        [`equipments/${z}`]: {
            name: z,
            img: "/assets/images/weapons/weapon_6.png",
            type: "weapon",
            classe: "Warrior",
            benefits: {
                damage: 3000,
                health: 1500,
                energy: 1500,
                spell_damage: 1500
            },
        }});

    // firebase.update({
    //     [`equipments/init_1`]: {
    //         name: "init_1",
    //         img: "/assets/images/weapons/init_1.png",
    //         type: "helmet",
    //         classe: "All",
    //         benefits: {
    //             damage: 100,
    //             health: 100,
    //             energy: 100,
    //         },
    //     }});
    //
    // firebase.update({
    //     [`equipments/init_2`]: {
    //         name: "init_2",
    //         img: "/assets/images/weapons/init_2.png",
    //         type: "armor",
    //         classe: "All",
    //         benefits: {
    //             damage: 100,
    //             health: 100,
    //             energy: 100,
    //         },
    //     }});
    //
    // firebase.update({
    //     [`equipments/init_3`]: {
    //         name: "init_3",
    //         img: "/assets/images/weapons/init_3.png",
    //         type: "boots",
    //         classe: "All",
    //         benefits: {
    //             damage: 100,
    //             health: 100,
    //             energy: 100,
    //         },
    //     }});
    //
    // firebase.update({
    //     [`equipments/init_4`]: {
    //         name: "init_4",
    //         img: "/assets/images/weapons/init_4.png",
    //         type: "weapon",
    //         classe: "All",
    //         benefits: {
    //             damage: 100,
    //             health: 100,
    //             energy: 100,
    //         },
    //     }});*/

  return {
    type: MAPTILE_COMPLETED,
    payload: {
      createdAt: now(),
      id: "",
      title: `MapTile #123`,
    },
  };
};




// CREATE ONLY
/*
export const maptileCompleted = () => ({ getUid, now }) => ({
    type: MAPTILE_COMPLETED,
    payload: {
      createdAt: now(),
      id: getUid(),
      title: `MapTile #123`,
    },
});
*/
export const saveMapTile = (maptile) =>  ({ firebase }) => {

  const { ...json } = maptile.toJS();
  const promise = firebase.update({
    [`maptile/${maptile.id}`]: { json },
  });
  return {
    type: FIREBASE_SAVE_MAPTILE,
    payload: promise,
  };
};
